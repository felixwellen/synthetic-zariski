...
with the boundary maps defined as expected, and its \v{C}ech cohomology $\check{H}^k(X,A)$ as the $k$-th homology group of its \v{C}ech complex. We can prove by induction on $n$ that $H^n(X,A) = \check{H}^n(X,A)$, and then use finite approximations to the \v{C}ech cover $2^\N\to \mathbb{I}$, we can check that $H^n(\mathbb{I},A) = 0$ for all $n>0$ and $A:\mathbb{I}\to \mathrm{Ab}_{\mathrm{cp}}$.
It is clear that $H^0(X,A) = \check{H}^0(X,A)$.
