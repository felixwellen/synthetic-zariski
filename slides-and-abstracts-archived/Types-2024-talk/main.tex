...
\begin{frame}{Topology on propositions}
  ...
  \begin{definition}
  A proposition is \only<-3>{closed}\only<4->{open} if it is a countable 
  \only<-3>{conjunction}\only<4->{{disjunction}} of decidable propositions. 
\end{definition}
\pause 
\begin{block}{
  \only<-3>{Closed} \only<4->{Open}
  propositions are expressable internally}
  A proposition $P$ is \only<-3>{closed}\only<4->{open}
  iff \pause there exists some $\alpha:2^\mathbb N$ with 
...
\begin{lemma}[using local choice]
  ...
  iff 
  it is a countable \only<-4>{intersection}\only<5->{{union}} of decidable subsets.
\end{lemma}
\end{frame}
