...
\bloc{Definition}{
A \v{C}ech cover of $X:\CHaus$ consists of $S:\Stone$ with $S\twoheadrightarrow X$.
}

\bloc{Definition}{
Given a \v{C}ech cover $S\twoheadrightarrow X$ and $A:X\to \Ab$, its \v{C}ech complex $\check{C}(X,S,A)$ is defined as
\[\prod_{x:X}A_x^{S_x}\to \prod_{x:X}A_x^{S_x^2} \to \prod_{x:X}A_x^{S_x^3}\to \cdots\]
}

\bloc{Definition}{
Given a \v{C}ech cover $S\twoheadrightarrow X$ and $A:X\to \Ab$, its \v{C}ech cohomology $\check{H}^n(X,S,A)$ is defined as the $n$-th cohomology of $\check{C}(X,S,A)$.
}
...
\bloc{Lemma}{
Given $S,T:\Stone,\, p:T\twoheadrightarrow S,\, A:S\to \Ab_\ODisc$, we have that 
\[\check{H}^1(S,T,A) = 0\]
}

\bloc{Proof}{
\begin{itemize}
\item Approximate $p$ as a sequential limits of maps between finite types.
\item We have $\check{H}^1(S_n,T_n,A)=0$ for $S$ finite.
...
\bloc{Proof}{
By induction on $n$. Assume given $\alpha:\prod_{x:S} K(A_x,n)$.
\begin{itemize}
...
is injective.
\item So $p^*:H^{n}(S,A)\to H^n(T,A\circ p)$ is injective, and it sends $\alpha$ to $0$.
\end{itemize}
}
...
%A \v{C}ech cover of $X:\CHaus$ consists of $S:X\to\Stone$ such that:
%\begin{itemize}
%\item $\prod_{x:X}\propTrunc{S_x}$
%\item $\Sigma_{x:X}S_x$ is Stone
%\end{itemize}

\bloc{Proof}{
From $0\to A_x\to A_x^{T_x}\to L_x\to 0$, we get long exact sequences:
...
We can prove that $H^n(\mathbb{I},A) = 0$ for all $n>0$, $A:\mathbb{I}\to \Ab_\ODisc$.

From this we get $\mathbb{I}$-localisation well-behaved.
