...
\bloc{Challenge}{
Find a class of $A$ such that $H^n(X,A)$ is well-behaved for $X:\CHaus$.
}
...
\bloc{Lemma (Tychonov)}{
For $I:\ODisc$ and $X:I\to \CHaus$, we have that $\Pi_{i:I}X_i$ is compact Hausdorff.
}

\bloc{Proposition (Tychonov dual)}{
For $X:\CHaus$ and $I:X\to \ODisc$, we have that $\Pi_{x:X}I_x$ is overtly discrete.
}
...
\item A morphism  from $(X,I)$ to $(Y,J)$ consists of $f:Y\to X$ with $\Pi_{y:Y} I_{f(x)}\to J_x$.
\end{itemize}
}
...
\bloc{Definition}{
A \v{C}ech cover of $X:\CHaus$ consists of $S:\Stone$ with $p:S\twoheadrightarrow X$.
}
...
\bloc{Lemma}{
Given $S,T:\Stone,\, T\twoheadrightarrow S,\, A:S\to \Ab_\ODisc$, we have that 
