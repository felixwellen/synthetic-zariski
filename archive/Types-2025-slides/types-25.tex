...
\begin{itemize}
\item It has open identity types.merely is the quotient of a countable set by an open equivalence relation.
...
\bloc{Definition}{
We have a category structure on $\Sigma_{X:\CHaus} (X\to \ODisc)$ where:
\begin{itemize}
\item A morphism $(X,I) \to (Y,J)$ consists of $f:Y\to X$ with $\forall y:Y.\ I_{f(x)}\to J_x$.
\end{itemize}
}
