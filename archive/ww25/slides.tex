...
\pause
\item We use Homotopy Type Theory now. This means we use a higher sheaf topos as a model.
\end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \(H^n(\Spec(A),M)=0\) for all \(M:\Spec(A)\to \Mod{R}_{\mathrm{wqc}}\).
    \pause
  \item \(H^n(X,M)\) computable with \v{C}ech-Cohomology, if \(X\) is a separated scheme.
    \pause
  \item \(\Aut(\bP^n)=\PGL_{n+1}(R)\).
    \pause
  \item \(\exists_{\text{ét}}\text{ isomorphism } X\cong \bP^n\) and \(X\) has a point, then there exists \(X\cong\bP^n\).
    \pause
  \item \(\Mod{R}_{\mathrm{ad}}\) is an abelian category, with self-duality through \(\Hom_{\Mod{R}}(\_,R)\), which might be a good replacement for coherent modules. \\
    \(\Mod{R}_{\mathrm{ad}}:=\) \\ \(\{K:\Mod{R}\mid \exists f:M\to N,\text{ $M$, $N$ fp}, K=\ker{f}\}\)
    \pause
  \item There is a variation of synthetic algebraic geometry for a synthetic theory of light condensed sets.
  \end{itemize}
\end{frame}
\end{document}
