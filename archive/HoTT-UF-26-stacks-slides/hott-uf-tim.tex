...
%Tim worked on Deligne-Mumford stacks in SAG.
...
%\begin{itemize} 
%\item Presheaves of $n$-groupoids are modelled as $n$-types.
%\item Being a sheaf is an accessible lex modality.
%\end{itemize}
%}
...
\spacebefore
\[R\mhyphen\Alg_{fp}  \simeq \{\text{Affine\ schemes}\}.\]
\spaceend
}

For affine schemes, \red{algebra and geometry match up perfectly}.
...
\bloc{Definition}{
We define the \red{projective space $\bP^n$} by
\spacebefore
\[\bP^n = \{\text{Lines\ in}\ R^{n+1}\ \text{going\ through}\ 0\}.\]
\spaceend
}

Any map from $\bP^n$ to $R$ is constant, so \red{$\bP^n$ cannot be affine}.

\pause

\bloc{Definition}{
A \red{scheme} is a set that has a finite open cover by affine pieces.
}
...
\bloc{Definition}{
A \red{cubic surface} is a scheme $X$ such that there exists $P:R[X_0,\cdots,X_3]$ homogeneous of degree $3$ with 
\spacebefore
\[X = \{x:\bP^3\ |\ P(x)=0\}.\]
\spaceend
}
...
\frame{\frametitle{Automorphisms of cubic surfaces}
...
\note{TODO}
The notion of Deligne-Mumford stack models this situation. 

\bloc{Intuition}{
A \red{Deligne-Mumford stack} is a $1$-type that is:
\begin{itemize} 
\item an étale sheaf.
...
\bloc{Remark}{
[Simpson 96] gave an extension to $n$-types.
}

Tim worked on a synthetic version of this extended version.

%Tim gave an extension to $n$-types, based on [Simpson 96].

%\bloc{Higher Deligne-Mumford stacks [Simpson 96]}{
%Types of Deligne-Mumford stacks are usually $2$-types, etc.
%}
...
\bloc{Traditional approach}{
Use sheaves of $n$-groupoids (aka $n$-stacks) on the \'etale site.
}

$\Rightarrow$ A lot of technical machinery is needed. \pause From [Simpson 96]:

\begin{center}
``In this paper we simply \emph{assume} that a theory of $n$-stacks of groupoids exists.''
\end{center}
...
\item Being an \'etale sheaf is an accessible lex modality.
\end{itemize}
}
...
\bloc{Definition}{
A type $X$ is \red{formally \'etale} if given any $\epsilon:R$ nilpotent, the map:
\spacebefore
\[X\to X^{\epsilon=0}\]
is an equivalence.
}
...
\bloc{Definition}{
A type $X$ is an \red{\'etale sheaf} if for all $S$ affine, formally \'etale and non-empty, we have that
\spacebefore
\[X\to X^{\propTrunc{S}}\]
is an equivalence.
}
...
%\bloc{Recall}{
If our goal is an \'etale sheaf, we can assume e.g.\ $X^2+1$ has a root\footnote{If $2$ is invertible in $R$.}.
%}
...
\bloc{Definition}{
An \red{\'etale cover} is a map whose fibers are formally \'etale and non-empty.
}
...
We give Tim's definition of the \red{class of Deligne-Mumford stacks}.
...
\item ($S$ affine, $p:S\to X$ \'etale cover whose fibers are in $\DM$) $\Rightarrow X\in\DM$.
\end{itemize}
}
...
\frame{\frametitle{General properties of Deligne-Mumford stacks}
...
\item $\DM$ is stable under \'etale quotients\footnote{Meaning given $Y$ an \'etale sheaf with $X\in \DM$ and $p:X\to Y$ an \'etale cover with fibers in $\DM$, we have that $Y\in \DM$.}.
\end{itemize}
}
...
\bloc{Definition}{
An \red{algebraic space} is a Deligne-Mumford stack which is a set\footnote{Traditionally, identity types in algebraic spaces are required to be schemes.}.
}
...
We want \red{non-trivial} examples of algebraic spaces.
...
\frame{\frametitle{An algebraic space that is not locally separated}
...
We define
\spacebefore
\[H = \{r:R^\times,\, x:R,\, y:R/x\ |\ y^2 = r\ \text{mod}\ x\}.\]
\spaceend
}
...
\begin{figure}
\includegraphics[width=0.5\textwidth]{images/tim.png}
\end{figure}

}


\frame{\frametitle{The future of Tim's work}

\begin{itemize}
\item A synthetic proof of smooth cubic surfaces forming a Deligne-Mumford $1$-stack has never been closer\footnote{Ongoing work with Felix Cherubini, Thierry Coquand, Fabian Endres, Jonas Höfer and Marc Nieper-Wi{\ss}kirchen}.
\item I am working on generalising Tim's definition to abstract sheaf models.
\item Tim's work should prove foundational for \'etale synthetic algebraic geometry.
\end{itemize}
