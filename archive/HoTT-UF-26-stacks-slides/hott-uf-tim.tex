...
For affines, algebra and geometry match up perfectly.
...
It cannot be a scheme, it is not even a set!
...
Gives a tight control on automorphisms groups of smooth cubics.
...
$\Rightarrow$ A lot of technical machinery need.
...
$\Rightarrow$ Simpler and more direct development.
...
\bloc{Definition}{
A type is $X$ is formally \'etale if given any $\epsilon:R$ nilpotent, the map:
...
\bloc{Definition}{
The \'etale topology $\mathbb{E}$ consists of formally \'etale and non-empty affine schemes.

%\subseteq \{\text{Affine}\}$ by

%\[(S\in\mathbb{E}) \leftrightarrow S\ \text{is\ formally\ \'etale\ and\ non-empty}\] 
}

\bloc{Definition}{
A type $X$ is an \'etale sheaf if for all $S:\mathbb{E}$, we have that
