...
\frame{\frametitle{A reminder: affine schemes}
...
\[\{f.p.\ R-algebra\} \simeq \{Affine schemes\}\]
\[A \mapsto Hom_{R-Alg}(A,R)\]
\[R^X \mapsto X\]
}

So when dealing with affine scheme, algebra and geometry match up perfectly.
...
\[\bP^n = \{Lines in R^{n+1} going through 0\}\]
}

Any map in $\bP^n\to R$ is constant, so $\bP^n$ cannot be affine for $n>0$.
...
\bloc{Definition}{
A cubic surface is a scheme $X$ such that there exists a $P:R[X_0,\cdots,X_3]$ homogeneous of degree $3$ with $X = \{x:\bP^3\ |\ P(x)=0\}$.
}

Consider the type of smooth cubic surfaces.

We have a surjective map $\bP^{19}\to \{Cubic surfaces\}$. Smoothness is an open condition.

\bloc{Block}{
The type of smooth cubics surfaces cannot be a scheme, it is not even a set!
}

}


\frame{\frametitle{27 lines}
...
\bloc{What about it?}{
This gives a tight control on automorphisms groups of smooth cubics.
}
...
Deligne-Mumford stacks is an appropriate generalisation of schemes to $1$-types, covering such moduli spaces.
...
\frame{\frametitle{Why synthetically?}

\bloc{Traditionally}{
We end up having to deal with sheaves of $n$-groupoids (aka $n$-stack) on the \'etale site.

$\to$ a lot of technical machinery is needed.
}

\bloc{Synthetically}{
\begin{itemize}
\item Sheaves of $n$-groupoids on the Zariski site are just $n$-types. 
\item \'Etale sheaves are just the modal types for some accessible lex modality.
\end{itemize}
}
...
\bloc{Definition}{
A type is $X$ is formally \'etale if given any $\epsilon:R$ such that $\epsilon^2=0$, the map:
\[X\to X^{x=0}\]
is an equivalence.
}
...
Intuitively, 
\begin{itemize}
\item $\epsilon^2=0$ means $\epsilon$ infinitesimal.
...
\bloc{Definition}{
The we define $\mathbb{T}$ the \'etale topology by:
\[\mathbb{T} = \{S Affine\ |\ S formally etale, not not S\}\]
}

\bloc{Definition}{
A type $X$ is an \'etale sheaf if for all $S:\mathbb{T}$, we have that:
\[X\to X^{\propTrunc{S}}\]
is an equivalence.
}
...
\frame{\frametitle{Stability properties}

\bloc{Proposition}{
Deligne-Mumford stacks are stable under the following:
\begin{itemize}
\item $\Sigma$ and identity types.
\item Covering quotients.
\end{itemize}
}

\bloc{Proposition (Descent for Deligne-Mumford stacks)}{
...
\bloc{Definition}{
An algebraic space is a set which is a Deligne-Mumford stack.
}

\bloc{Warning!}{
Tradition requires identity types to be schemes
}

\bloc{Intuition}{
A scheme is a set that is Zariski locally affine.

An algebraic space is a set that is \'etale locally affine.
}

}


\frame{\frametitle{A chain of strict inclusions}

\[\{Schemes\} \subsetneq \{Locally separated algebraic space\} \subseteq \{Algebraic space\}\]
...
\bloc{Definition}{
Given $x:R^\times$, we define:
\[C_x = \{z:R\ |\ z^2 = x\}\]
\[D_x = \Sigma_{y:R} C_x^{y=0}\]
}

We have $p:D_x\to R$ such that its fiber of $y=0$ is $C_x$, its fiber over $y\not=0$ is $1$.

\bloc{Lemma}{
Consider $H=\Sigma_{x:R^\times} D_x$, then:
\begin{itemize}
