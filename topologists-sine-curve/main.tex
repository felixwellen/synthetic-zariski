...
\end{lemma}
\begin{proof}
  For each $r:[\frac1k , 1]$, we can define a Cauchy sequence approximating $\sin(\frac1r)$. 
  Recall also that the Cauchy reals agree with our reals by LLPO and dependent choice, thus for each $r$, we can define $t(r)$. Thus we can define $t$. 
\end{proof}
\begin{lemma}
  In the same setting, we can define for each $n:\mathbb N$ the standard metric $d:\mathbb R^n \times \mathbb R^n \to \mathbb R_{\geq 0}$.
\end{lemma}
\begin{proof}
  Similar as above. 
\end{proof}
...
\begin{lemma}\label{postiveNumbersCharacter}
  For $x:\mathbb I$ with $x\neq 0$ there exists some $k:\mathbb N$ with $x>\frac1k$. 
\end{lemma}
\begin{proof}
