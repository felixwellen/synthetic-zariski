...
\begin{lemma}\label{cofinaltbseq}
  Given $r:[0,1]$ with $r>0$, there are some $k:\N$ with $t_k,b_k<r$.  
\end{lemma}
\begin{proof}
  By \Cref{positiveNumbersCharacter}, there is some $k:\N$ with $\frac1k<r$. As $(2k+\frac12)\pi>k$, this $k$ suffices, it follows that $t_k<r$, and as $b_k<t_k$ we have $b_k<r$ also. 
\end{proof}
...
\begin{theorem}[The topologist's sine curve is not path-connected]
  There is no map $\gamma:\mathbb I \to Y$ such that $\gamma(0) = (1,sin(1))$ and $\gamma(1) = (0,0)$. 
\end{theorem}
\begin{proof}
  By applying dependent choice to \Cref{SineWillHitTopBot}, 
  we can find increasing sequences $u_i,v_i,~i:\N$ such that $\pi_2(\gamma(u_i)) = 1, \pi_2(\gamma(v_i)) = -1$ and 
  $u_{i+1}>v_{i+1}>u_i>v_i$ for all $i:\mathbb N$. 
  By \Cref{uniformContinuity} for $\epsilon = 2$, there is some $\delta:[0,1]$ with $\delta>0$ such that 
  $d(u_i, v_j)\geq \delta$ for any $i,j:\mathbb N$. 
  Therefore, the sequence $u_i$ is increasing with steps of at least size $2\delta>0$. 
  But this means that for $N>\frac1{2\delta}$, we have $u_{N}>1$, while $u_N :[0,1]$, which is a contradiction. 
\end{proof}
