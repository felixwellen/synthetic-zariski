...
In \cite{synthetic-stone-duality}, the axioms of synthetic Stone duality were introduced. Within that system, one could prove Markov's principle and the lesser limited principle of omniscience (LLPO). We could also define a synthetic topology on any type. These principles and topology worked well together, and we will recall and extend some results below in order to make this note self-contained. 
...
\begin{proof}
  ...
  For $\epsilon = 2$, there is some $\delta>0$ such that $d(u_i, v_j)\geq \delta$ for any $i,j:\mathbb N$ as  $d(\gamma(u_i), \gamma(v_j)) > 2$. 
