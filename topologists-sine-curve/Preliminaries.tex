...
\begin{definition}
  Let $X$ be a type. $X$ is called \textbf{strongly locally compact} \parencite[p59]{lesnik2021synthetictopologyconstructivemetric} if there exists some overt $\mathcal B$ 
  and map $b = (b_O,b_K) : \mathcal B \to ((X \to \Open) \times (Comp(X)))$ such that:
  \begin{itemize}
...
\end{definition} 
\begin{lemma}
  Compact Hausdorff spaces are strongly locally compact. 
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{corollary}
  By \parencite[Prop 2.59]{lesnik2021synthetictopologyconstructivemetric}, it follows that compact Hausdorff spaces have a canoncical basis and that if $X,Y:\CHaus$, with presentations $q:S \twoheadrightarrow X, p:T \twoheadrightarrow Y$, then $X\times Y$ has the canonical product topology with $\mathcal B = 2^S \times 2^T$ and $b(D, E) = (\neg q(D)) \times (\neg p(E))$. 
\end{corollary}
