\rednote{This chapter will be factored out to a different draft and will probably be removed from this one in the near future. Also: The name "Adelman" is outdated and was changed to "finitely adequate" to stay in line with the literature.}
...
\begin{lemma}
  ...
  Let $A$ and $C$ be finitely co-presented $R$-modules, $K$ be a Adelman-module and $e:A\to K$ a epimorphism.
...
\begin{proof}
  ...
  Let $A$ and $C$ be given by $A=\Spec R[X_1,\dots,X_n]/(P_1,\dots,P_r)$ and $C=\Spec R[Y_1,\dots,Y_m]/(Q_1,\dots,Q_s)$.
  ...
  \]
  which induces a linear map $f_L:A\to C$.
...
\begin{lemma}
  ...
  The cokernel of a homomorphism between Adelman-modules is Adelman.
\end{lemma}
...
\begin{proof}
  By \Cref{coherent-kernels} Adelman-modules are closed under cokernels.
  By the duality from \Cref{coherent-self-dual}, this also shows the closure under kernels.
\end{proof}
...
Using concepts like microlinearity, we can show that Adelman modules are closed under extensions.
Recall from \cite{diffgeo-article} that for any finitely presented $R$-module $M$, the disk of $M$ is the affine variety $\mathbb{D}(M) = \Spec (R \oplus M)$, where $R \oplus M$ is the square zero extension algebra.
Note that
$\mathbb{D}(R^n) = \Spec (R[X_i]_{1\leq i \leq n}/(X_i X_j)_{1 \leq i \leq j \leq n})$.
This is also equivalent to the $n$-disk, which we define to be
$\mathbb{D}(n) = \{ x : R^n \mid xx^T = 0\}$,
where $x$ is thought of as a column vector, so $x x^T$ denotes the outer product.

For any $m,n : \mathbb{N}$ we define the following inclusions
\begin{align*}
  \mathrm{inl}_{m,n} : \mathbb{D}(m) & \rightarrow \mathbb{D}(m + n) \\
  \varepsilon & \mapsto (\varepsilon,0)
\end{align*}
\begin{align*}
  \mathrm{inr}_{m,n} : \mathbb{D}(n) & \rightarrow \mathbb{D}(m + n) \\
  \varepsilon & \mapsto (0,\varepsilon)
\end{align*}
\begin{align*}
  \mathrm{inc}_{m,n} : \mathbb{D}(m) \underset{\ast}{\coprod} \mathbb{D}(n) & \rightarrow \mathbb{D}(m+n) \\
  \mathrm{inl}(\varepsilon) & \mapsto \mathrm{inl}_{m,n}(\varepsilon) \\
  \mathrm{inr}(\varepsilon) & \mapsto \mathrm{inr}_{m,n}(\varepsilon)
\end{align*}
We can also define a diagonal on the $1$-disk
\begin{align*}
  D : \mathbb{D}(1) & \rightarrow \mathbb{D}(2) \\
  \varepsilon & \mapsto (\varepsilon,\varepsilon)
\end{align*}
For brevity, we introduce the following notation for pointed mapping spaces: $(Y,y)^{(X,x)} := (X,x) \rightarrow_\ast (Y,y)$. If the pointing of $(X,x)$ is understood from context, we write $(Y,y)^X$ instead.

\begin{definition}
  Let $X$ be a type and $x : X$ a point of $X$.
  $X$ is \notion{infinitesimally linear at $x$} if for every $m, n : \mathbb{N}$ the induced map
  \begin{align*}
    \mathrm{inc}_{m,n}^* : (X,x)^{\mathbb{D}(m+n)} \rightarrow (X,x)^{\mathbb{D}(m) \underset{*}{\coprod} \mathbb{D}(n)}
  \end{align*}
  is an equivalence.
\end{definition}

Any scheme is infinitesimally linear at all of its points \cite{diffgeo-article}.
In particular, finite free modules and finitely copresented modules are infinitesimally linear at any point.

% Why doesn't this work?  It does in other LaTeX documents.
% \begin{proposition}[{\cite[5.0.1]{david-orbifolds}}]
\begin{proposition}[5.0.1 \cite{david-orbifolds}]
...
\begin{example}
  The modules $R[x] = R^R$ and $R[[x]] = R^\mathbb{N}$ are infinitesimally linear at the constant function $0$. Since automorphisms of the underlying types act transitively, they are infinitesimally linear at any point.
\end{example}

The converse of the above result is not true in general.
\rednote{I'm not sure which statement you are considering the converse of.}
For $R$-modules, we can show that cokernels of infinitesimally linear types at $0$, are themselves infinitesimally linear.
To do so, we will need to talk about infinitesimal varieties.

\begin{definition}[\cite{etale-draft}]
  \phantom{M} \\
  \begin{enumerate}[(a)]
  \item A \notion{Weil algebra} over $R$ is a finitely presented $R$-algebra $W$ together with a homomorphism $\pi : W \to R$ such that the kernel of $\pi$ is a nilpotent ideal.
  \item An \notion{infinitesimal variety} is a pointed type $D$ such that $D = (\Spec W, \pi)$ for a Weil algebra $(W, \pi)$.
  \end{enumerate}
\end{definition}

Given a finitely presented $R$-module $M$, then its disk $\mathbb{D}(M)$ is an infinitesimal variety.

\begin{lemma}[Lemma 10.0.1 \cite{etale-draft}]\label{infinitesimal-projective}
  Let $X$ be an infinitesimal variety.
  Then $X$ is projective among types.
\end{lemma}

\begin{proposition}\label{projective-exact}
  Let $(X,x)$ be a pointed object which is projective among types.
  Then the functor $(\underline{\phantom{Y}})^{(X,x)}$ is exact for $R$-modules.
\end{proposition}

\begin{proof}
  Let $M \rightarrow N$ be an epimorphism of $R$-modules, and suppose we have a pointed map $f : (X,x) \rightarrow_\ast (N,0)$.
  We merely get a lift $\hat{g} : X \rightarrow M$ of $f$ along the epi.
  The map $g = \hat{g} - \hat{g}(0)$ is a pointed lift of $f$, as desired.
\end{proof}

For every $n : \mathbb{N}$ the type $\mathbb{D}(n)$ is an infinitesimal variety.
We can thus deduce by the results above that for any $m,n : \mathbb{N}$ the functors $(\underline{\phantom{Y}})^{\mathbb{D}(m+n)}$ and $(\underline{\phantom{Y}})^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} = (\underline{\phantom{Y}})^{\mathbb{D}(m)} \times (\underline{\phantom{Y}})^{\mathbb{D}(n)}$ are exact.

\begin{proposition}
  Cokernels and extensions of infinitesimally linear $R$-modules at $0$ are themselves infinitesimally linear at $0$.
\end{proposition}

\begin{proof}
  This follows by exactness of the functors $(\underline{\phantom{Y}})^{\mathbb{D}(m+n)}$ and $(\underline{\phantom{Y}})^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)}$.
  In both cases, just apply the $5$-lemma to deduce that $\mathrm{inc}_{m,n}^\ast$ is an equivalence, as illustrated in the diagrams below.
  \begin{center}
    \begin{tikzcd}
      A^{\mathbb{D}(m+n)} \ar[hook]{d} \ar{r}[above]{\simeq}[below]{\mathrm{inc}_{m,n}^\ast} & A^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} \ar[hook]{d} \\
      B^{\mathbb{D}(m+n)} \ar{r}[above]{\simeq}[below]{\mathrm{inc}_{m,n}^\ast} \ar[two heads]{d} & B^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} \ar[two heads]{d} \\
      C^{\mathbb{D}(m+n)} \ar{r}[above]{}[below]{\mathrm{inc}_{m,n}^\ast} & C^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)}
    \end{tikzcd}
    \qquad
    \begin{tikzcd}
      A^{\mathbb{D}(m+n)} \ar[hook]{d} \ar{r}[above]{\simeq}[below]{\mathrm{inc}_{m,n}^\ast} & A^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} \ar[hook]{d} \\
      B^{\mathbb{D}(m+n)} \ar{r}[above]{}[below]{\mathrm{inc}_{m,n}^\ast} \ar[two heads]{d} & B^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} \ar[two heads]{d} \\
      C^{\mathbb{D}(m+n)} \ar[]{r}[above]{\simeq}[below]{\mathrm{inc}_{m,n}^\ast} & C^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)}
    \end{tikzcd}
  \end{center}
  \phantom{M}
\end{proof}
...
\begin{corollary}
  Adelman modules and their extensions are infinitesimally linear at $0$.
\end{corollary}

\begin{proof}
  This is immediate by the above proposition, as Adelman modules are kernels of cokernels of infinitesimally linear $R$-modules at $0$.
\end{proof}

Recall that the \notion{tangent space} $\mathrm{T}_x X$ of a type $X$ at the point $x : X$ is the type of pointed maps $(X,x)^{\mathbb{D}(1)}$.
This defines a functor, where a map $f : (X, x) \rightarrow (Y, y)$ is taken to its derivative $df_x : \mathrm{T}_xX \rightarrow \mathrm{T}_y Y$ which is defined by post-composition of pointed maps.
% To be a functor, I think we need the input map to be pointed.
By \Cref{infinitesimal-projective} and \Cref{projective-exact},
the functor $\mathrm{T}_0$ is exact on modules and $R$-linear maps, since $\mathbb{D}(1)$ is an infinitesimal variety.

Let $M$ be an $R$-module.
The tangent space $\mathrm{T}_0M$ admits an $R$-module structure by taking pointwise addition and scalar multiplication on the tangent vectors.
This module structure is called the pointwise module structure on $\mathrm{T}_0M$.
We define a map $\mathrm{can} : M \rightarrow \mathrm{T}_0M$ by $m \mapsto (r \mapsto r \cdot m)$, and call this the canonical map.
It is clear that the canonical map is $R$-linear.

\begin{proposition}
  The family of maps $\mathrm{can} : M \rightarrow \mathrm{T}_0M$ is natural with respect to $R$-linear maps. \hfill$\square$
\end{proposition}

\begin{proposition}
  The map $\mathrm{can} : R^n \rightarrow \mathrm{T}_0R^n$ is an equivalence of types.
\end{proposition}

\begin{proof}
  By duality, the tangent space $\mathrm{T}_0R^n$ consists of those polynomials in $p(x) : R[x]/(x^2)$ such that $p(0)=0$.
  These polynomials are precisely those of the form $\mathrm{can}_r$ for any $r : R$.
\end{proof}

\begin{corollary}\label{can-equiv}
    The map $\mathrm{can} : M \rightarrow \mathrm{T}_0M$ is an equivalence on Adelman modules and their extensions.
\end{corollary}

\begin{proof}
  Since $\mathrm{T}_0$ is an exact functor, this follows from the above proposition and the $5$-lemma.
\end{proof}

\begin{theorem}[Thm 1.1.10 \cite{etale-draft}]
  Let $X$ be an infinitesimally linear type at $x : X$.
  Then the tangent space $\mathrm{T}_xX$ admits an $R$-module structure.
  This module structure will be called the microlinear module structure on $\mathrm{T}_xX$. \hfill$\square$
\end{theorem}

We can describe the relevant binary operations of the microlinear module structure.
Suppose we are given two tangent vectors $\upsilon, \nu : \mathrm{T}_0M$, then $\upsilon \boxplus \nu$ (the microlinear addition) is defined by the composite $(\mathrm{inc}_{1,1}^\ast)^{-1}[\upsilon,\nu] \circ D$.
In other words, we obtain this map by lifting in the following diagram
\begin{center}
  \begin{tikzcd}
    & \mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1) \ar{rd}{[\upsilon,\nu]} \ar{d}{\mathrm{inc}_{1,1}} \\
    \mathbb{D}(1) \ar{r}{D} \ar[bend right]{rr}{\upsilon \boxplus \nu} & \mathbb{D}(2) \ar[dashed]{r}{\exists !} & M
  \end{tikzcd}
\end{center}
Scalar multiplication by $r : R$ is defined by $(r \cdot \upsilon)(\varepsilon) = \upsilon(r\varepsilon)$ for any $\varepsilon : \mathbb{D}(1)$.

If $f : X \rightarrow Y$ is a map of infinitesimally linear types, then the derivative $df_x$ is $R$-linear with respect to the microlinear module structure.

\begin{theorem}
  Suppose that $M$ is an infinitesimally linear $R$-module at $0$. Then the map ${\mathrm{can} : M \rightarrow \mathrm{T}_0M}$ is $R$-linear for the microlinear module structure on $\mathrm{T}_0M$.
\end{theorem}

\begin{proof}
  It is clear that for any $r : R$ the maps $r\cdot \mathrm{can}(m)$ and $\mathrm{can}(rm)$ are the same.
  For any $\varepsilon : \mathbb{D}(1)$ we get $(r\cdot\mathrm{can}(m))(\varepsilon) = \mathrm{can}(m)(r\varepsilon) = r\varepsilon m = \varepsilon rm = \mathrm{can}(rm)(\varepsilon)$.

  Because $\mathrm{can}$ is $R$-linear for $\mathrm{T}_0M$ with the pointwise module structure, it suffices to show that the microlinear and pointwise addition on $\mathrm{T}_0M$ are the same.
  The following argument uses Eckmann-Hilton to do so.
  It is clear that the identity for both of these $R$-module structures is the map constant on $0$, so it remains to argue that the maps are compatible.
  Let $+$ denote the addition on $M$, and we use the same symbol for the pointwise module structure on $\mathrm{T}_0M$.
  Let $\boxplus$ denote the microlinear addition on $\mathrm{T}_0M$.
  Our goal is to show that for any $\alpha, \beta, \gamma, \delta : \mathrm{T}_0M$, $(\alpha + \gamma) \boxplus (\beta + \delta) = (\alpha \boxplus \beta) + (\gamma \boxplus \delta)$ holds.

  The map $(\alpha + \gamma) \boxplus (\beta + \delta)$ is given by lifting in the following diagram
  \begin{center}
    \begin{tikzcd}
      & \mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1) \ar{d}{\mathrm{inc}_{1,1}} \ar{r}{[\Delta,\Delta]} & \mathbb{D}(1)^2 \underset{*}{\coprod} \mathbb{D}(1)^2 \ar{rr}{[\alpha \times \gamma, \beta \times \delta]} & & M^2 \ar{d}{+} \\
      \mathbb{D}(1) \ar{r}{D} \ar[bend right, looseness = 0.5]{rrrr}{(\alpha + \gamma) \boxplus (\beta + \delta)} & \mathbb{D}(2) \ar[dashed]{rrr}{\exists !} & & & M
    \end{tikzcd}
  \end{center}
  We get $(\alpha \boxplus \beta) + (\gamma \boxplus \delta)$ by lifting in the following diagram
  \begin{center}
    \begin{tikzcd}
      & & (\mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1))^2 \ar{d}{\mathrm{inc}_{1,1}\times \mathrm{inc}_{1,1}} \ar[bend left]{rd}{[\alpha,\beta] \times [\gamma,\delta]} \\
      \mathbb{D}(1) \ar{r}{\Delta} \ar[bend right, looseness = 0.5]{rrrr}{(\alpha \boxplus \beta) + (\gamma \boxplus \delta)} & \mathbb{D}(1)^2 \ar{r}{D \times D} & \mathbb{D}(2)^2 \ar[dashed]{r}{\exists} & M^2 \ar{r}{+} & M
    \end{tikzcd}
  \end{center}
  By using the naturality square involving $\Delta$ and $D$ we can glue the lower parts of the diagrams together.
  \begin{center}
    \begin{tikzcd}
      \mathbb{D}(1) \ar{r}{D} \ar{d}{\Delta} & \mathbb{D}(2) \ar[dashed, bend left, looseness = 0.5]{rrd}{\exists !} \ar{d}{\Delta}\\
      \mathbb{D}(1)^2 \ar{r}{D \times D} & \mathbb{D}(2)^2 \ar[dashed]{r}{\exists} & M^2 \ar{r}{+} & M
    \end{tikzcd}
  \end{center}
  ...
  Since $M$ is infinitesimally linear, this is the case iff pre-composing with $\mathrm{inc}_{1,1}$ makes the triangle commute.
  Using the naturality square involving $\Delta$ and $\mathrm{inc}_{1,1}$, this happens if the outer arrows of the following diagram commutes
  \begin{center}
    \begin{tikzcd}
      \mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1) \ar{r}{\Delta} \ar{d}{\mathrm{inc}_{1,1}} & (\mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1))^2 \ar{d}{\mathrm{inc}_{1,1}\times \mathrm{inc}_{1,1}} \ar[bend left]{rd}{[\alpha,\beta] \times [\gamma,\delta]} \\
      \mathbb{D}(2) \ar{r}{\Delta} \ar[dashed, bend right, looseness = 0.5]{rrr}{\exists !} & \mathbb{D}(2)^2 \ar[dashed]{r}{\exists} & M^2 \ar{r}{+} & M
    \end{tikzcd}
  \end{center}
  One can now observe that this square commutes because the following square does
  \begin{center}
    \begin{tikzcd}
      \mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1) \ar{r}{[\Delta, \Delta]} \ar{d}{\Delta} & (\mathbb{D}(1) \underset{*}{\coprod} \mathbb{D}(1))^2 \ar{d}{[\alpha, \beta]\times [\gamma,\delta]} \\
      \mathbb{D}(1)^2 \underset{*}{\coprod} \mathbb{D}(1)^2 \ar{r}{[\alpha\times \gamma, \beta\times \delta]} & M^2
    \end{tikzcd}
  \end{center}
...
We are now ready to prove that Adelman modules are closed under extension.

\begin{theorem}
  Adelman modules are closed under extension.
\end{theorem}

The following strategy was first proposed by Felix Cherubini, but the linearizing trick was not known at that point. David Wärn independently discovered this together with an idea on how to linearize the maps.

\begin{proof}
  By \Cref{adelman-ext-problem-red}, it suffices to show that
  $\textup{Ext}^1_R(P,I) = 0$ when $P$ is finitely copresented and $I$ is finitely presented.
  In fact, we'll show this just assuming that $I$ is Adelman.
  So suppose that $I \hookrightarrow E \overset{p}{\twoheadrightarrow} P$ is a short exact sequence, with $P$ finitely copresented and $I$ Adelman.
  Our goal is to split $E$ as $I \oplus P$, i.e.\ to find a linear section of $p$.

  Since $I$ is Adelman, it is also wqc.
  Therefore $p$ admits a (possibly non-linear) section $s$ by the same argument as (i) in \Cref{maps-into-coherent-lift}.
  Consider the diagram involving the derivative of $s$ and the naturality square of $\mathrm{can}$ for the linear map $p$:
  \begin{center}
    \begin{tikzcd}
      \mathrm{T}_0E \ar[bend left]{d}{dp_0} & E \ar{d}{p} \ar{l}[above]{\simeq}[below]{\mathrm{can}} \\
      \mathrm{T}_0P \ar[bend left]{u}{ds_0} & P \ar{l}[above]{\simeq}[below]{\mathrm{can}}
    \end{tikzcd}
  \end{center}
  By \Cref{can-equiv}, the top map is an equivalence.
  Since $s$ is a section of $p$, $ds_0$ is a section of $dp_0$, so
  we obtain a linear section of $p$ by taking $\mathrm{can}^{-1}\circ ds_0 \circ \mathrm{can}$.
\end{proof}
...
$R^U$ is Adelman, which implies that $K^U$ and $R/K^U$ are as well.
\rednote{Citation?}

For any $U$, it's easy to see that the canonical map $R^U \to T_0(R^U)$ is an
equivalence, and therefore this follows for $K^U$ and $R/K^U$ as well.
(This is tentatively called being ``tangential''.)
...
When $U$ is a proposition, all five are not not Adelman, since they
are Adelman when $U$ is true or false.
...
Note that $\ker \varphi_r$ is finitely copresented, and so is Adelman.
Therefore, $\im \varphi_r$ is Adelman as well.
...
Since $V(r)$ is closed, the modules in the bottom row are Adelman.
...
Given some $r : R$ that is either invertible or nilpotent, we can deduce that $R^{\lnot D(r)}$ and $K^{\lnot D(r)}$ are Adelman.
The following proposition tells us that we cannot assert that $R^{\lnot D(r)}$ is Adelman for all $r : R$.
...
\begin{proof}
  ...
  The global sections of the bundle ${r \mapsto R^{\lnot D(r)}}$ form the power series module: $R[[x]] = \prod_{r : R} R^{\lnot D(r)}$ \rednote{Citation needed}.
...
\item Show that the map $\mathrm{can} : M \rightarrow \mathrm{T}_0M$ is not an equivalence.
...
\item Adelman implies not not finite-free \rednote{Citation?},
so it suffices to prove that $M$ is not finite free.
To show $\lnot (\text{for all $r : R$, $M_r$ is Adelman})$ for a family $M_r$,
