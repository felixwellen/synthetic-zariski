...
\begin{lemma}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  The difference kernel $K = \mathrm{Ker}(u) \cap \mathrm{Ker}(f)$ is the kernel of the induced map $ \mathrm{Ker}(u) \rightarrow \mathrm{Ker}(v)$.
\end{lemma}

\begin{proof}
  Observe that $\mathrm{Ker}(u) \subseteq \mathrm{Ker}(gu)$.
  Thus $\mathrm{Ker}(u) = \mathrm{Ker}(u) \cap \mathrm{Ker}(gu)$.
...
\begin{proposition}
  \label{exact-seq-comm-sq}
  Given a commutative square as above, there is an exact sequence
  \begin{center}
    \begin{tikzcd}
      0 \ar{r} & K \ar[hook]{r} & A \ar{r} & B \oplus C \ar[two heads]{r} & B \underset{A}{\oplus} C \ar{r} & 0
    \end{tikzcd}
  \end{center}
\end{proposition}

\begin{proof}
  This is an immediate consequence of \Cref{diff-ker-is-ker}.
\end{proof}

\begin{remark}
We also get a dual exact sequence
\begin{center}
  \begin{tikzcd}
    0 \ar{r} & B \underset{D}{\times} C \ar[hook]{r} & B \oplus C \ar{r} & D \ar[two heads]{r} & L \ar{r} & 0
  \end{tikzcd}
\end{center}
where $L$ is the cokernel of the induced map $\mathrm{Cok}(u) \rightarrow \mathrm{Cok}(v)$.
\end{remark}
...
\begin{proof}
  Suppose that $K$ is the kernel of a map $M \rightarrow N$, where $M$ and $N$ are finitely copresented.
  By \Cref{presentation-extension}, we can find a linear extension
  \begin{center}
    \begin{tikzcd}
      K \ar[hook]{d} \\
    ...
    \end{tikzcd}
  \end{center}
  By \Cref{exact-seq-comm-sq}, $K$ is also the kernel of the map $R^{m_1} \rightarrow R^{m_2 + n_1}$.
\end{proof}
...
\begin{center}
  ...
  \end{tikzcd}
\end{center}
By \Cref{exact-seq-comm-sq}, we obtain the following exact sequence
\begin{center}
  \begin{tikzcd}
