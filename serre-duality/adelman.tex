...
In particular, finite free $R^n$ and finitely copresented $P$ modules are infinitesimally linear at any point.
...
The tanget space $\mathrm{T}_0M$ admits an $R$-module structure by taking pointwise addition and scalar multiplication on the tangent vectors.
...
\begin{proposition}
  The family of maps $\mathrm{can} : M \rightarrow \mathrm{T}_0M$ is natural on the $R$-linear maps. \hfill$\square$
\end{proposition}
...
\begin{proof}
  By duality, the tanget space $\mathrm{T}_0R^n$ consists of those polynomials in $p(x) : R[x]/(x^2)$ such that $p(0)=0$.
  These polynomials are precisely those on the form $\mathrm{can}_r$ for any $r : R$.
\end{proof}
...
\begin{proof}
  Since $\mathrm{T}_0$ is an exact functor we can conclude with the above proposition and the $5$-lemma.
\end{proof}
