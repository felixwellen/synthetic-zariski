...
Recall from \cite{diffgeo-article} that for any fintely presented $R$-module $M$, the disk of $M$ is the affine variety $\mathbb{D}(M) = \Spec (R \oplus M)$, where $R \oplus M$ is the square zero extension algebra.
We will also write $\mathbb{D}(n) = \mathbb{D}(R^n) = \Spec (R[X_i]_{1\leq i \leq n}/(X_i^2)_{1 \leq i \leq n})$.
The $n$-disk admits a simpler description: $\mathbb{D}(n) = \{ x : R^n \mid xx^T = 0\}$.
...
\begin{lemma}[Lemma 10.0.1 \cite{etale-draft}]
...
\begin{proof}
  ...
  The map $g = \hat{g} - g(0)$ is a pointed lift $f$, as we desired.
\end{proof}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      A^{\mathbb{D}(m+n)} \ar{d} \ar{r}[above]{\simeq}[below]{\mathrm{inc}_{m,n}^\ast} & A^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)} \ar{d} \\
      ...
      C^{\mathbb{D}(m+n)} \ar[dashed]{r}[above]{}[below]{\exists !} & C^{\mathbb{D}(m)\underset{\ast}{\coprod}\mathbb{D}(n)}
    \end{tikzcd}
    \qquad
    \begin{tikzcd}
...
Recall that the \notion{tangent space} of a type $X$ at the point $x : X$ is the type of pointed maps $(X,x)^{\mathbb{D}(1)}$.
This defines a functor, where a map $f : X \rightarrow Y$ is taken to its derivative $df_x : (X,x) \rightarrow (Y,f(x))$.
It is clear that this functor is exact on modules, since $\mathbb{D}(1)$ is an infinitesimal variety.
...
\begin{theorem}
  Suppose that $I \hookrightarrow E \overset{p}{\twoheadrightarrow} P$ is a short exact sequence, with $P$ finitely copresented and $I$ Adelman.
  Then $E$ is Adelman as well.
\end{theorem}
...
\begin{proof}
  Our goal is to split $E$ as $I \oplus P$, i.e. finding a linear section of $p$.
  It then follows that $E$ is Adelman, as Adelman-modules are closed under direct sums.

  Since $I$ is Adelman, it is also wqc.
  Therefore $p$ admits a section $s$ by the same argument of (i) in \cref{maps-into-coherent-lift}.
  consider the diagram by glueing the derivative of $s$ to the naturality square of $\mathrm{can}$ against the linear map $p$.
  \begin{center}
    \begin{tikzcd}
