...
\begin{proof}
  By \Cref{diff-ker-is-ker} we can replace $K$ with $K'$, in which case the sequence is immediate.
\end{proof}

\rednote{As \Cref{exact-seq-comm-sq} was stated before (using the other kernel) it was trivial and didn't require \Cref{diff-ker-is-ker}.
In fact, it didn't depend on $g$, $v$ or $D$ at all.
I've now stated it with the other kernel.
Looking at uses, we never actually use the last term in the sequence, so in fact we don't
need this result at all and instead can just use \Cref{diff-ker-is-ker}.  So should we delete this?
(Thomas, delete these comments when read and handled.)}
...
\rednote{Need to say a few words about why $K^{D(r)}$ has that description, or cite something.}
...
\begin{equation*}
  ...
  \end{tikzcd}
\end{equation*}
This has the same top row as \eqref{eq:compare-Vr}.
\textbf{I haven't thought about the bottom row here.}
Does it have a nice description?
How does it compare to the bottom row of \eqref{eq:compare-Vr}?
