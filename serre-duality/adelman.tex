...
\begin{center}
  ...
  \end{tikzcd}
\end{center}
\rednote{Can you explain how the next sentence follows from the previous diagram?  Which pushout square are you talking about?}
...
\begin{lemma}
  ...
  \rednote{Should ``fcp'' be ``Adelman'' here?}
\end{lemma}
...
\begin{proof}
  ...
  \rednote{I don't follow.  This would show that $P \otimes Q$ is the kernel of a map between finitely \textbf{co}presented modules.  But don't we need it to be the kernel of a map between finitely presented modules, or the \textbf{co}kernel of a map between finitely copresented modules?}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  As $\textup{Tor}_1(\cdot, \cdot)$ is balanced, we can compute it starting from $P$, but then we see that $\textup{Tor}_1(P,M) \subseteq 0$, so the map $P \otimes Q \rightarrow P \otimes R^{n_1}$ is monic. \rednote{Why ``$\subseteq$'' and not ``=''?}
\end{proof}
