...
\begin{remark}
  ...
  In classical algebraic geometry, it is not possible to define the cotangent sheaf as the dual of the tangent sheaf.
  This means that the synthetic dualization of module-bundles does \emph{not} agree with classical dualization of $\mathcal O_x$-module sheaves.
  One difference is that the dualization we use respects the scheme structure of finitely copresented $R$-modules.
  More concretely, let $V$ be a finitely presented $R$-module, then by the universal property of the symmetric algebra $\mathrm{Sym}^\ast V$ of $V$, we have:
  \[
    V^\star = \Hom_{\Mod{R}}(V,R) = \Hom_{\Alg{R}}(\mathrm{Sym}^\ast V,R) = \Spec(\mathrm{Sym}^\ast V)
  \]
  So $(V^\star)^\star$ is the type of maps $(\Spec(\mathrm{Sym}^\ast V)\to R)=\mathrm{Sym}^\ast V$ which happen to be $R$-linear.
\end{remark}
