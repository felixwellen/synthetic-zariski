...
See \cite{Hartshorne}[p. 172] or \cite{vakil}[p. 573] for the classical theory.
...
$df$ for $f : A$, subject to relations $d(rf) = r \cdot df$ for $r : R$ and
...
\[\D(M^\star) \to \D(N^\star)\]
which by \Cref{equivalence-module-infinitesimal} gives a linear section of $f$. \rednote{TODO should we check functoriality?}
\end{proof}
