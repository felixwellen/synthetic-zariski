...
(iii) implies (i): Indeed given $\epsilon:R$ such that $\epsilon^2=0$, assume $x,y:X$ such that $\epsilon=0 \to x=y$. Then $x\in N_1(y)$ and by \Cref{duality-infinitesimal-tangent} and $T_y(X)=0$ we conclude $x=y$.
\end{proof}
...
\begin{proof}
  By \Cref{etale-zariski-local}, it is enough to consider an affine scheme $X=\Spec(R[X_1,\cdots,X_n]/P_1,\cdots,P_k)$.
  For any $x:X$, by \Cref{kernel-is-tangent-of-fibers} we have an exact sequence: 
  \begin{center}
    \begin{tikzcd}
      0\ar[r] & T_x(X)\ar[r] & R^n\ar[r] & R^k
    \end{tikzcd}
  \end{center}
  By \Cref{unramified-affine-characterisation}, $X$ is unramified if and only if $T_x(X)=0$ for all $x:X$.
