...
\begin{proof}
By \Cref{etale-zariski-local} and \Cref{standard-smooth-is-smooth}, we get the converse. 
...
From \Cref{smooth-have-free-tangent}we get that for any $x:X$ we have that $dP_x$ has finite free kernel of rank $k$. Then by \Cref{rank-equivalent-definitions} we know that $dP_x$ has rank $n=m-k$ for every $x$.
...
so that $\left(\frac{\partial P}{\partial X}\right)_{x,y}$ is invertible of size $q$. Moreover by \Cref{rank-bloc-matrix} we get:
...
\begin{center}
      \begin{tikzcd}
        R/(Q_1(x,y),\cdots,Q_l(x,y)) & \Spec(R[X_1,\cdots,X_q,Y_1,\cdots,Y_k]/P_1,\cdots,P_q,Q_1,\cdots, Q_l)\ar[l,swap,"(x{,}y)"] \ar[dashed,ld,"(x{'}{,}y{'})"]\\
       R\ar[u] & \\
      \end{tikzcd}
\end{center}
