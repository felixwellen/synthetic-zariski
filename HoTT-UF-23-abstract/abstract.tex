...
a version quatifying over external algbras is called the \emph{comprehensive axiom}\footnote{
  In \cite{kock-sdg}[I.12], Kock's ``axiom $2_k$'' could equivalently be thoerem 12.2,
...
is very convienient to work with.
However, classicilally, basic open subsets of an affine scheme are given
...
With Zariski-local choice, we were able to show that these notions of openess agree in our setup.
...
We did not explore, if an analogous setup can be used for derived algebraic geometry
...
which we want to study in future work, as an example application.
