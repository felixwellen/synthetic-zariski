...
of this presheaf. For any set theoretic universe $\UU$, we have a corresponding presheaf ${\cal U}$ which to
...
ring $k$.
    ...
    equality $F\times F\rightarrow\Omega$ for each type/presheaf $F$.
    ...
    We have a strict truncation operation $F\mapsto \norm{F}_S,~{\cal U}\rightarrow\Omega$, which to any presheaf $F$ on $\int Yo(B)$
    ...
    Any representable presheaf is a sheaf by algebraic facts\rednote{... by algbraic facts.}. If $B$ is a finitely presented $k$-algebra
    ...
    We present another model which both interprets a cumulative hierarchy of universes\rednote{s?}
