...
\begin{definition}\label{DfnEntourage}
  An \textbf{entourage} of a type is an open reflexive relation. 
\end{definition}
\subsection{Shrinking of entourages}
\begin{remark}
  We can immediately note that entourages are closed under intersection, and each entourage contains the diagonal. Furthermore, it is clear that if $E$ is an entourage, then so is $E^{-1}$. 
\end{remark}
In order to be a fundamental system of entourages in the sense of \parencite[170]{BourbakiTopologyCh1To4}, it remains to be shown that whenever $E$ is an entourage, we can shrink it to another entourage $F$ such that $F\circ F \subseteq E$. 
\begin{definition}
...
\end{definition}
\begin{remark}
  Note that a map $U:I \to X^\Open$ is a cover if and only if $\Delta \subseteq \bigcup_{i:I} U_i \times U_i$. 
\end{remark}

\begin{lemma}\label{ReduceToFinite}
...
\end{definition}
\begin{lemma}\label{ReducesToDelta}
  Let $X$ be a type with covers $(U_i)_{i:I},(V_j)_{j:J}$. If $U$ is a $\Delta$-refinement of $V$, then 
  $$\left(\bigcup_{i:I} U_i \times U_i\right) \circ \left(\bigcup_{i:I} U_i \times U_i\right) \subseteq \bigcup_{j:J} V_j \times V_j.$$
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}\label{closedSubcover}
...
\end{lemma}
\begin{proof}
  We may assume that $I = \mathbb N_{\leq n}$. We will define for each $l:\mathbb N_{\leq n+1}$ and each $0\leq i < l$ a sequence $V_i \subseteq D_i \subseteq U_i$ with $D_i$ closed and $V_i$ open, and such that
  \begin{equation}\label{inductionLoop}\bigcup_{0 \leq i < l} V_i \cup \bigcup_{l\leq  i\leq n} U_i = X.\end{equation}
...
%
  Assume $V_i,D_i$ have been defined for $i<k$ and be such that \Cref{inductionLoop} is satisfied for $l=k$. 
  Note that $\bigcup_{i<k} V_i \cup \bigcup_{k<i} U_i$ is open, and hence $\neg\neg$-stable, and define $C_k$ as its complement. By the induction hypothesis we have $C_k \subseteq U_k$. By \Cref{seperation}, we find $V_k,D_k$ open and closed respectively such that 
  $C_k \subseteq V_k \subseteq D_k \subseteq U_k$. 
  Now
  $$
  \bigcup_{0\leq i< k+1} V_i \cup \bigcup_{k+1\leq i \leq n} U_i = 
  (\bigcup_{i<k}V_i \cup 
  \bigcup_{k<i} U_i)
  \cup V_k 
  = 
  ...
  Thus \Cref{inductionLoop} holds for $l=k+1$, and we conclude by induction. 
\end{proof}

The following is Lemma 2.2 of \cite{Howes}, which is based on a proof by Morita \cite{MoritaStar}. 
...
\end{lemma}
\begin{proof}
  As $2^I$ is finite, each $B_J$ is indeed open (resp. closed). 
  By \Cref{SubsetUnion}, we have $X = U_i \cup \neg C_i$ for all $i:I$. Then also
  $X = \bigcap_{i:I} (U_i \cup \neg C_i)$. 
  But by distributivity of conjunction and disjunction, this also equals
%  $\bigcup_{J:2^I} B_J$,
  $
  \bigcup_{J:2^I} \left(\bigcap_{j:J} U_j \cap \bigcap_{j:\neg J} \neg C_j\right),
  $
  from which it follows that $B$ is an open (resp. closed) cover.
  % 
...
\begin{theorem}[Shrinking]\label{shrinking}
  Let $X:\CHaus$ and let $E$ an entourage of $X$. Then there exists an entourage $F$ of $X$ with $F\circ F \subseteq E$. 
\end{theorem}
\begin{proof}
  By \Cref{ReduceToFinite} we may assume $E = \bigcup_{i:I} U_i \times U_i$ for some finite open cover $(U_i)_{i:I}$ of $X$. 
  By lemma \Cref{ReducesToDelta}, we only have to give a $\Delta$-refinement of $U$, which by \Cref{Morita} reduced to finiding a closed subcover of $U$, which is given by \Cref{closedSubcover}.
\end{proof}
