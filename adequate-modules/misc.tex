...
\item What are examples of modules that are not finitely adequate?
      ...
      % For a proof we give a constructivized version of the one given in \cite[Proposition 3.2]{conner2021inverselimitslendergroups}, which seems to be folklore.
      % The proof in loc.cit.\ works over $\mathbb Z$ and makes use the prime factorization, which we replace by a neat degree-counting argument using duality.
      ...
      It is enough to show that the induced map
        ...
        \[
          f(r) = a_0 + a_1r + a_2r^2 + \dots + a_{N-1}r^{N-1} + t^N\lambda(0,\dots,0,1,r,r^2,\dots)\rlap{,}
        \]
        where exactly the first $N$ entries in $\lambda(0,\dots,0,1,r,r^2,\dots)$ are zero.
        As our goal is a proposition and $f$ a polynomial, we may suppose that $\deg f_0 \leq d$ for some $d \geq 0$.
      ...
      \begin{corollary}
        The $R$-module $R^{\mathbb{N}}$ of sequences in $R$ is reflexive but not weakly quasi-coherent.
        Hence it also fails to be finitely adequate.
      \end{corollary}


      \begin{proof}
        We already know that $R^{\mathbb{N}}$ is not weakly quasi-coherent.
        By the previous lemma we see that it is not reflexive.
        \rednote{TODO: Re-check that the above pairing really induces the double-dual map.}
      \end{proof}
