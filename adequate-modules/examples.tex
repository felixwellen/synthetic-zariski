...
\rednote{Maybe we don't actually want to talk about characteristic in the following counterexample at all? All we need is that there is a prime $p:\mathbb N$ such that $p=0:R$.}

\begin{counterexample}\label{cex:hood-p-not-tangential}
  Suppose that $R$ has positive characteristic $p > 0$ and let $n > 0$ be arbitrary.
  Assume further that $p$ is prime\footnote{
    As $R$ is a local ring, an assumed characteristic might only be the power of a prime number.
  },
  ensuring that $\hood{R,0}{p^n-1}$ is a submodule of $R$.
  Then we have a chain of inclusions $\hood{R,0}{1} \subseteq \hood{R,0}{p^n-1} \subsetneq R$.
...
For any proposition $U$, the modules $R_U$ are tangential.
\end{proposition}
...
\begin{example}\label{ex:K^V(r)-fin-adeq}
  ...
  By \Cref{ex:exp-Vr-fin-adeq}, $R^{V(r)} = \quot{R}{\ideal{r}}$.
  Thus, $K^{V(r)} = \ideal{r}$, which is finitely adequate by \Cref{ex:fin-gen-ideal}.
\end{example}
...
\begin{proof}
  We need to show that the predicates are equivalent.
...
\begin{counterexample}\label{cex:R_V(r)-not-reflexive}
  ...
  Therefore, we get that for any $x, r : R$ satisfying $xr = 0$, either $x = 0$ or $r = 0$.
...
\begin{counterexample}\label{cex:K-not-Dr-not-wqc}
  ...
  This gives $(r \text{ is nilpotent}) \lor (r \text{ is invertible})$ for every $r : R$, which contradicts the connectedness of $R$.
\end{counterexample}
...
\begin{proof}
  By \Cref{prp:fin-adeq-cok-closure}, \quot{R}{I} is finitely adequate.
  And by \Cref{cor:dual-equiv}, $\Ann{I} = (\quot{R}{I})^*$ is finitely adequate.
\end{proof}

\begin{remark}\label{rem:ann-for-cex}
  By the contrapositive of \Cref{prp:ann-fin-adeq}, if $\Ann{I}$ is not finitely adequate, then we can deduce that $I$ is not finitely adequate.
\end{remark}

\begin{counterexample}\label{cex:R_not-Dr-not-fin-adeq}
  It is not the case that $R_{\neg D(r)}$ is finitely adequate for all $r : R$.
  By \Cref{lem:ann-R_U-K^U} we have $\Ann(R_{\neg D(r)}) = K^{\neg D(r)}$.
  By \Cref{rem:ann-for-cex} and \Cref{cex:K-not-Dr-not-wqc} we know that $R_{\neg D(r)}$ cannot be finitely adequate for all $r : R$.
\end{counterexample}

\begin{counterexample}\label{cex:K^Dr-not-fin-adeq}
  It is not the case that $K^{D(r)}$ is finitely adequate for all $r : R$.
  Since $R^{D(r)} = R[r^{-1}]$, we get that $K^{D(r)} = \bigcup_k \Ann(r^k)$.
  By \Cref{lem:ann-R_U-K^U}, we know that $\ideal{r^k} = \Ann(R_{V(r^k)})$, and thus $\Ann(\Ann(r^k)) = \ideal{r^k}$.
  Moreover, it is clear that taking annihilator ideals of unions, is the same as the intersection of the annihilators.
  Thus we get that $\Ann(K^{D(r)}) = \Ann(\bigcup_k \ideal{r^k}) = \bigcap_k \Ann(r^k) = \bigcap_k \ideal{r^k} = K^{\neg D(r)}$.
  Now, by \Cref{rem:ann-for-cex} and \Cref{cex:K-not-Dr-not-wqc} we know that $K^{D(r)}$ cannot be finitely adequate for all $r : R$.
\end{counterexample}

\begin{counterexample}\label{cex:R_Dr-not-fin-adeq}
  The module $R_{D(r)}$ is not finitely adequate for all $r : R$.
  This is precisely because of \Cref{lem:ann-R_U-K^U}, \Cref{rem:ann-for-cex}, and \Cref{cex:K^Dr-not-fin-adeq}.
\end{counterexample}
...
\begin{proof}
  ...
  Comparing coefficients, we see that $a_N=0$ for $N \geq d+1$, hence $a$ is an element of $R[x]$.
...
\begin{remark}
  ...
  \begin{enumerate}
    ...
    It is however worthwhile to understand that, at least as of now, it is only clear that the evaluations of $\Gamma(X, \mc F \otimes \mc G)$ are jointly injective (resp. surjective).
...
\begin{proof}
  ...
  \[
  \begin{tikzcd}
    \Gamma(X,\mc F') \otimes_A \Gamma(X,\mc G) \ar[d, "\vartheta''_X"] \ar[r] & \Gamma(X,\mc F) \otimes_A \Gamma(X,\mc G) \ar[d, "\vartheta_X"] \\
    \Gamma(X, \mc F' \otimes \mc G) \ar[r] & \Gamma(X, \mc F \otimes \mc G)
  \end{tikzcd}
  \]
  where $\vartheta''_X$ is the obvious map.
  We know that it is an injection by the preceding case.
  The lower horizontal map need not be injective.
  However, by construction of $\mc F'$, it is so for the particular element $\vartheta''_X(k')$.
  Hence $\vartheta_X(k) = 0$ implies $\vartheta''_X(k') = 0$ and then $k' = 0$.
