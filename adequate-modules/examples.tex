...
\begin{remark}
  ...
  In particular, such an $M$ is tangential if and only if it is finitely adequate.
  \rednote{Why does tangential imply finitely adequate in this case?
  For the converse, cite the result in this paper.}
\end{remark}

Consider \Cref{prp:inf-lin-hood}, from which we can extract the following principle: if there is a chain of inclusions $\hood{M}{0} \subseteq N \subseteq M$, then $M$ is infinitesimally linear at $0$ if and only if $N$ is.

\begin{example}
  As an example, $\mathrm{Nil}(R)$ is infinitesimally linear at $0$ since it contains $\hood{R}{0} = \D(1)$.
\end{example}
\rednote{Should $\hood{X}{0}$ be $\hood{X}{1}$ above and below?  Also, be consistent about whether the point $0$ is included, as in $\hood{X,0}{1}$.  Maybe we can always skip it for a pointed type?}

On the other hand, the above principle behaves very differently when we consider tangential modules.
Suppose there is a chain of proper inclusions $\hood{M_n,0}{1} \subset M_0 \subset M_1 \subset \dots \subset M_n$.
Then, by \Cref{prp:hood-factorization}, we have $\tangent{M_i}{0} = \tangent{M_n}{0}$ for all $i \leq n$.
Thus, at most one of the $R$-modules $M_i$ can be tangential.
\rednote{This is equally strong with just two modules $N$ and $M$, so better to state it that way as then it parallels the previous comment about infinitesimal linearity.}
...
Most modules considered so far are submodules of finitely presented modules.
Nevertheless, infinitely generated modules can also be tangential.
\rednote{The previous two sentences don't contrast each other correctly, since a submodule of a finitely presented module isn't necessarily finitely generated (I think).}
We present two such examples below.
...
\begin{example}\label{ex:power-series-tangential}
  ...
  Since automorphisms of the underlying types act transitively, they are infinitesimally linear at any point.
  \rednote{Is the previous sentence worth mentioning here?  It's true for any $R$-module (or abelian group), right?  So we should say it once when infinitesimal linearity is introduced.}
...
\begin{example}\label{ex:exp-Vr-fin-adeq}
  Let $r : R$.
  The module $R^{V(r)} = \quot{R}{\ideal{r}}$ is finitely adequate by \Cref{ex:fin-gen-ideal}.
\rednote{Also cite closure under cokernels.}
\rednote{We should be consistent about the notation for ideals.  Round brackets are used a lot,
including in \Cref{ex:fin-gen-ideal}.}
\end{example}
...
\begin{counterexample}\label{cex:exp-not-Dr-wqc}
  ...
  However, $R[[x]]$ is not finitely adequate by \cite[Proposition A.0.5]{draft}, which is a contradiction.
\rednote{Why does the last sentence say ``finitely adequate''?}
\rednote{Should we add that it follows that it is not the case that $R^{\lnot D(r)}$ is finitely adequate for all $r$?}
\end{counterexample}
