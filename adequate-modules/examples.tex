...
\begin{counterexample}\label{cex:K^Dr-not-fin-adeq}
  ...
  By \Cref{lem:ann-R_U-K^U}, we know that $\ideal{r^k} = \Ann(R_{V(r^k)})$, and thus $\Ann(\Ann(r^k)) = \ideal{r^k}$ as is the case for every annihilator ideal.
  \rednote{If it's the case for every annihilator, why do we give a roundabout proof?}
