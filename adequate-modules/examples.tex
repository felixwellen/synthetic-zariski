...
\begin{example}
  The module $K^{V(r)}$ is finitely adequate for any $r : R$.
  By \Cref{ex:exp-Vr-fin-adeq}, $R^{V(r)} = \quot{R}{r}$.
...
\begin{proof}
  ...
  Let $x : K^U$ and $y : R_U$.
  We need to show that $xy = 0$.
  Since we're proving a proposition, we can assume that either $y = 0$ or $U$ holds by definition of $R_U$.
...
For the next counterexample, we will use the above observations to show the module $R_{V(r)}$ is not finitely adequate for any $r : R$.
...
\begin{counterexample}\label{cex:K-not-Dr-not-wqc}
  It is impossible to prove that $K^{\lnot D(r)}$ is weakly quasi-coherent for every $r : R$.
  In particular, it is impossible to prove that $K^{\lnot D(r)}$ is finitely adequate for every $r : R$.
...
\begin{counterexample}\label{cex:R_not-Dr-not-fin-adeq}
  The module $R_{\neg D(r)}$ is not finitely adequate for all $r : R$.
...
\begin{counterexample}\label{cex:K^Dr-not-fin-adeq}
  The module $K^{D(r)}$ is not finitely adequate for all $r : R$.
...
Applying this lemma to affine schemes of the form $D(r)$, we obtain the following.
