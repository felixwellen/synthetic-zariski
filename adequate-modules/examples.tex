...
\begin{proof}
  We can define a map $\Ann(R_U) \to K^U$ given by $x \mapsto x$.
  To fill out the proof that this map is well-defined, we will assume that $U$ holds, and the goal is to show that $x = 0$.
  If $U$ holds, then $R_U = R$.
  Thus, $\Ann(R_U) = \Ann(R) = 0$, meaning $x = 0$ as desired.

  This is an injective linear map by definition, so to see that it defines an isomorphism, it suffices to check that it is surjective.
  Let $x : K^U$ and $y : R_U$.
  We need to show that $xy = 0$ to assert that $x$ is a term of $\Ann(R_U)$.
...
\begin{proof}
  Let $f : \quot{R}{I} \to R$ be a linear map.
  Then $f$ is uniquely determined by $f(\bar{1}) = a : R$.
  By linearity we get that $ay = f(\bar{y}) = 0$ for any $y : I$.
  Thus $a : \Ann(I)$.

  Conversely, let $a : \Ann(I)$.
  Define the map $f' \defeq (\argmhole) \cdot a : R \to R$.
  Since $f'(y) = ay = 0$ for any $y : I$, $f'$ factors through $\quot{R}{I}$ as the map $f : \quot{R}{I} \to R$.

  It is clear that these two operations are inverses to each other.
  This proves that $\Ann(I) = (\quot{R}{I})^*$.
\end{proof}

For the next counterexample, we will use the above observations to show that it is not the case that $R_{V(r)}$ is finitely adequate for all $r : R$.

\begin{counterexample}\label{cex:R_V(r)-not-reflexive}
  To show that $R_{V(r)}$ is not finitely adequate for all $r : R$, we will compute the double dual of $\quot{R}{R_{V(r)}}$.
  By \Cref{lem:ann-dual-quotient}, we have $\Ann(R_{V(r)}) = (\quot{R}{R_{V(r)}})^*$.
  Furthermore, by \Cref{lem:ann-R_U-K^U}, we have $\Ann(R_{V(r)}) = K^{V(r)} = \ideal{r}$.
  Since $\ideal{r}$ is self-dual, we get that $(\quot{R}{R_{V(r)}})^{\ast\ast} = \ideal{r}$.

  For the sake of contradiction, suppose that $R_{V(r)}$ is finitely adequate.
  Then $\quot{R}{R_{V(r)}}$ is finitely adequate by \Cref{prp:fin-adeq-cok-closure}.
  Since taking duals is an anti-equivalence on finitely adequate modules (\Cref{cor:dual-equiv}), we get that $(R_{V(r)})^{\ast\ast} = \Ann(r)$.
  By assumption, $R_{V(r)}$ is finitely adequate for any $r : R$, so the evaluation map $\ev : R_{V(r)} \to \Ann(r)$ is an equivalence.
  This means that for any $x, r : R$ such that $xr = 0$, then either $x = 0$ or $r = 0$.
