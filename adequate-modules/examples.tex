...
\begin{counterexample}\label{cex:K^Dr-not-fin-adeq}
  ...
  \rednote{I don't follow the previous sentence.}
  By \Cref{lem:ann-R_U-K^U}, we know that $\ideal{r^k} = \Ann(R_{V(r^k)})$, and thus $\Ann(\Ann(r^k)) = \ideal{r^k}$ as is the case for every annihilator.
