...
So far, the most effective method for finding counterexamples is not to construct a specific non finitely adequate module, but instead to show that, within a given family of modules, not all the members are finitely adequate.
...
\begin{remark}
  ...
  In particular, every such $M$ is tangential if and only if it is finitely adequate.
\end{remark}
...
\begin{counterexample}\label{cex:exp-not-Dr-wqc}
  It is impossible to prove that the module $R^{\lnot D(r)}$ is weakly quasi-coherent for every $r : R$.
...
We first show that these modules are tangential.
...
Since $R[[x]]$ is not finitely adequate, we see that the class of reflexive $R$-modules and the class of finitely adequate $R$-modules are different.
...
In this subsection, we show that the tensor product of two linearly adequate modules is not linearly adequate.
This shows that the double dual tensor is the appropriate left adjoint to consider for the internal hom of $\finadeq$.
