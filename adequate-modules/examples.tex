...
\subsection{Temporary storage for former A.1 and A.2}

\begin{lemma}\label{lem:finitely-adequate-not-fp}
  Not every finitely adequate $R$-module is either finitely presented or finitely copresented.
  Specifically, this is not the case for the family of ideals $(r)$, for $r:R$.
\end{lemma}

\begin{proof}
  We first show that if $(r)$ is finitely copresented, then $(r=0) \lor (r \neq 0)$. Indeed, finitely copresented modules are projective in the category of finitely adequate $R$-modules, so we get a lift
  \[
  \begin{tikzcd}
...
\begin{remark}\label{rem:finitely-adequate-fp}
  ...
  A similar statement holds for finitely copresented.
\end{remark}
