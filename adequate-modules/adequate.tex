...
We recall quasi-coherence from the foundations paper % \cite{foundation-sag}. This key makes everything crash for some reason....
...
\rednote{This is Ingo's concept, maybe say something about that?}
...
\rednote{These pointed maps are already linear. Does linearity makes sense when the domain is $\D (n)$? Respects scalar multiplication and addition whenever it exists.}

\begin{lemma}\label{lem:disk-qc}
  Let $Q$ be a quasi-coherent $R$-module.
  A pointed map $p : (Q,0)^{\D (n)}$ is a polynomial in $Q$ with degree $1$ and constant term $0$.
\end{lemma}
...
\begin{definition}\label{def:adeq}
  Let $M$ be an $R$-module.
  $M$ is \notion{adequate} if it is the kernel of a linear map $f : P \rightarrow Q$ between quasi-coherent modules $P$ and $Q$.
\end{definition}
...
\begin{proof}
  By \cref{prp:lim-col-ext-inf} and \cref{cor:tangent-like-qc}.
\end{proof}
...
\begin{remark}\label{rem:lin-adeq}
  In the article \cite{diffgeo-article} linearly adequate modules went under the name finitely copresented module.
  We decided to change the name here as linearly adequate matches the external concept.
\end{remark}

\begin{definition}\label{def:dual}
  Let $M$ be an $R$-module.
  We define the \notion{dual} of $M$ to be the module $M^\ast \defeq \Hom_{\modcat{R}}(M,R)$.
\end{definition}

\begin{theorem}[Lemma 2.3.9, Corollary 2.3.10\cite{diffgeo-article}]\label{thm:lin-adeq-dual}
  ...
  Moreover, the canonical map into the double dual, $L\to L^{\ast\ast}$ is an isomorphims for finitely presented and linearly adequate $R$-modules. \earlyqed
\end{theorem}
...
Linearly adequate modules are closed under kernels.
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  By \Cref{lem:diff-ker-is-ker}, $K$ is also the kernel of the map $R^{m_1} \rightarrow R^{m_2 + n_1}$.
\end{proof}
...
\begin{proof}
  By \Cref{lem:fp-lift}, we can assume $f$ is induced by a square of morphisms between finite free $R$-modules, and we construct the linearly adequate modules $A$ and $B$ like described below:
  \begin{center}
  \begin{tikzcd}
...
\begin{definition}\label{def:fin-adeq}
  Let $M$ be an $R$-module, then $M$ is a \notion{finitely adequate module} if one of the following equivalent statements hold:
  \begin{enumerate}[(i)]
...
\begin{proposition}\label{prp:fin-adeq-adeq}
  Any finitely adequate module is adequate.
  In particular, finitely adequate modules are also tangent-like and wqc.
\end{proposition}

\begin{proof}
  Any finitely presented module is quasi-coherent.
  Since any finitely adequate module is merely the kernel of a map between quasi-coherent modules, it is itself adequate.
...
\begin{proof}
  By \cite{draft}[Theorem 7.3.6] we have $H^1(L,M)=0$, since $L$, like any linearly adequate $R$-module, is an affine scheme.
...
\begin{proof}
  ...
  $M$ is now linearly adequate as it is a kernel of linear adequates, \Cref{prp:lin-adeq-ker-closure}.
\end{proof}
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  Since the composite $M \rightarrow P^{\ast\ast}$ is a monomorphism, so has $\ev : M \rightarrow M^{\ast\ast}$ to be.
...
\begin{remark}\label{rem:fin-adeq-Adelman}
  By \cite[Theorem 1.15]{adelman-construction}, \Cref{thm:lin-adeq-proj} and \Cref{thm:fp-inj} implies that the category $\finadeq$ is the free abelian category over the finite free $R$-modules.
  This means the finitely adequate $R$-modules have the following universal property:
  For every abelian category $\mathcal{A}$ and all additive functors $F:\finfree \to \mathcal{A}$, there exists a unique exact functor $K$ like follows:
  \begin{center}
    \begin{tikzcd}
...
\begin{proposition}\label{prp:fin-adeq-hom-closure}
  For finitely adequate $R$-modules $M,N$, the $R$-module $\Hom_R(M,N)$ is a finitely adequate $R$-module as well.
\end{proposition}
...
\begin{lemma}\label{lem:hood-module-prs-surj}
  Let $f : M \rightarrow N$ be a epimorphism of $R$-modules.
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  By \cite{diffgeo-article}[Lemma 2.1.4] and \Cref{thm:lin-adeq-dual}, the morphisms in the top square extends to modules.
...
Finitely adequate modules forms a weak Serre subcategory of $\modcat{R}$.
...
\rednote{Should one either introduce Ext with a reference to Dan and Jarl's paper, or should we write out this? We are not using any special properties, it is just a convenient way to prase things...}

\begin{lemma}\label{lem:fin-adeq-ext-red}
  Finitely adequate $R$-modules are closed under extension if and only if for any linearly adequate $R$-moduke $L$ and finitely adequate $R$-module $B$, $\textup{Ext}^1_R(L,B) = 0$.
\end{lemma}
...
\begin{proof}
  ...
  This amount to showing that any lifting problem
  \begin{center}
    \begin{tikzcd}
      & E \ar[two heads]{d}{p} \\
      L \ar[equal]{r} & L
...
Thus it suffices to show that the tensor of linearly adequate $R$-modules is finitely adequate to show that all finitely adequate $R$-modules are closed under tensors.
...
\begin{proof}
  ...
  It suffices to show that the map $P \otimes Q \rightarrow P \otimes R^{n_1}$ is monic, because $P \otimes R^{n_i}$ is linearly adequate for $i = 0$ and $1$.
