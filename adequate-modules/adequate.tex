...
\begin{proof}
  ...
  \rednote{To me the third isomorphism theorem is about quotient and not kernels... Maybe say: Since the kernel of a square can be computed both column wise or row wise, the following diagram gives the desired isomorphism?}
...
\end{lemma}
\rednote{Proposal:  Change this to the statement:
``For every linearly adequate $R$-module $L$ and finitely adequate $R$-module $B$, every exact sequence $0 \to B \to E \to L \to 0$ splits.''
Then exchange this proof and the proof of the next theorem.
Alternatively, just fold this result into the proof of the next theorem.}
...
\begin{proof}
  ...
  Assume now that $\textup{Ext}^1(L,B) = 0$ for all such modules $L$ and $B$.
  ...
  Since we assumed that any exact sequence of the form $0 \to B \to E \to L_1 \to 0$ splits, it is necessary that $p^*B = B \oplus L_1$, which is finitely adequate.
...
\begin{proof}
  By \Cref{lem:fin-adeq-ext-red} it is sufficient to show that every exact sequence of the form $0 \to B \to E \to L \to 0$ splits when $B$ is finitely adequate $B$ and $L$ is linearly adequate.
...
\begin{proof}
  ...
  The second map is an equivalence due to reflexivity of $M$ and the last due to duality.
...
\begin{proof}
  ...
  This is equivalent to showing that $M^\ast$ is projective, i.e.\ we need to show that $H :\equiv \Hom_R(M^\ast,\argmhole)$ is exact on the category of finitely adequate modules.
