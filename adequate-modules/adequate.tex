...
\begin{proof}
  ...
  Then $d_0f=d_0g$ and by naturality of the map $\can$ the claim follows.
...
\begin{remark}
  ...
  In the stated generality however, this is no longer true.
  {
    \renewcommand{\Ne}{\ensuremath{N_\varepsilon}}

    Indeed, for $\varepsilon:\D(1)$ and $\Ne : \equiv R/\ker(R\overset{\cdot \varepsilon}{\to}R)$ we have that $\Ne = \hood{\Ne,0}{1}$, since for every $n:\Ne$ we have $(\varepsilon=0)\to(n=0)$.
    Hence it suffices to show that it is not the case, that for all $\varepsilon:\D(1)$ the well-defined map $\alpha_\varepsilon : \Ne \to \Ne, [r] \mapsto [r^2]$ is $R$-linear.
    From $\alpha_\varepsilon([1])=[1]$ and assuming linearity, we would get $[r]=[r^2]$ which implies that $\varepsilon(r^2-r)=0$ for every $r:R$.
    Using duality and comparing coefficients, we see that $\varepsilon=0$, which cannot be.
  }
\end{remark}
