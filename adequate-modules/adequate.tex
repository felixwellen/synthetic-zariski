...
\begin{proof}
  ...
  We deduce that $\can(q) = qx$, so the canonical map $\can$ is an equivalence by assumption.
\end{proof}
...
\begin{proof}
  By \cite[Theorem 7.3.6]{draft} we have $H^1(L,M)=0$, since $L$, like any linearly adequate $R$-module, is an affine scheme. Indeed, as the kernel of a linear map $f : R^m \to R^n$, the module $L$ is equivalent to $\Spec(\quot{R[x_1, \dots, x_m]}{\ideal{f(x_1), \dots, f(x_n)}})$.
