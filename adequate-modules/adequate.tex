...
\begin{proof}
  Every finitely presented $R$-module is quasi-coherent \rednote{I think we should give a reference or an explanation}.
...
We say that $M$ is \notion{reflexive} is this map is an equivalence.
...
\begin{proof}
 Consider an extension problem:
  ...
  where $A$ and $B$ are finitely adequate. We merely get a presentation $L' \rightarrow L \overset{p}{\twoheadrightarrow} A$ with $L$ and $L'$ linearly adequate. Consider the pullback square:
    ...
    \end{tikzcd}
  \end{center}
  By \Cref{lem:fin-adeq-ext-pb-red} is is enough to show that $p^*E$ is finitely adequate, but the kernel of the top map is equal to $B$, so we have a new extension problem:
    \begin{center}
    \begin{tikzcd}
...
\begin{proof}
  ...
  Thus the canonical map $\vartheta_X'' : \Gamma(X,\mc F') \otimes_A \Gamma(X,\mc G) \to \Gamma(X, \mc F' \otimes \mc G)$ is injective by the preceeding case.
