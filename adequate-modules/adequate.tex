...
\begin{lemma}
  ...
  The $M$ is quasi-coherent if and only if for every finitely presented $R$-algebra $A$ the map
  $\beta_{M,A}$ is an equivalence.
  Here $A$ is regarded as an $R$-module.
\end{lemma}

\begin{proof}
  Suppose $M$ is reflexive and consider the following composition:
  \begin{align*}
  ...
  We omit the verification that this composition agrees with the map $\beta_{M,A}$.
  Thus the first map is an equivalence if and only if the composition is.
\end{proof}
