...
For the remainder of this section, we restrict our attention to a subcategory of adequate modules whose objects satisfy certain finiteness conditions.
We will start by discussing the concept which is dual to that of finitely presented modules, which was first introduced in \cite{diffgeo-article}.

\begin{definition}\label{def:lin-adeq}
  An $R$-module $L$ is called \notion{finitely copresented} if it is the kernel of a linear map $f : R^m \rightarrow R^n$.
  We refer to the map $f$ as a \notion{finite copresentation} of $L$.
\end{definition}

\begin{remark}\label{rem:lin-adeq}
  Finitely copresented modules have been studied externally.
  In this setting they are called linearly adequate modules \cite[Chapter 06Z1]{stacks-project}.
\end{remark}
...
\begin{proof}
  ...
  Assume now that $M$ is projective, and let $L$ be a finitely copresented module such that there is an epimorphism $p : L \twoheadrightarrow M$.
  ...
  \rednote{Don't we need to \emph{show} that there is such an epi from a finitely copresented module, rather than assuming that there is?}
\end{proof}
...
However, even taking the tensor product $M \otimes N$ of finitely copresented modules (which is a stronger condition) fails to be finitely adequate (see \Cref{cex:tensor-not-closed}).
...
\begin{proof}
  ...
  Using Zariski-local choice and boundedness \cite[Theorem 3.3.3]{draft}, while shrinking $X$ if necessary as explained in the first paragraph, we get integers $p \geq n$ and $q \geq 0$ together with $S : \Gamma(X, \mc F)^{1 \times p}$, $T : \Gamma(X, \mc G)^{p \times 1}$, $U : \Gamma(X, \mc G)^{q \times 1}$ and $G : \Gamma(X, R)^{p \times q} \simeq A^{p \times q}$ such that $SG = 0$ and $GU = T$.
