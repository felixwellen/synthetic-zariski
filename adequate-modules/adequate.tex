...
\begin{lemma}\label{lem:disk-qc}
  ...
  A pointed map $(Q,0)^{\D(n)}$ is an $n$-variate linear monomial with coefficients in $Q$. More precisely, the map $Q^n\to (Q,0)^{\D(n)}$ sending $(q_1,\dots,q_n)$ to $(\epsilon_1,\dots,\epsilon_n)\mapsto \sum_{i=1}^n\epsilon_iq_i$ is an equivalence.
\end{lemma}
...
\begin{proof}
  The case $n=1$ in \Cref{lem:disk-qc} indeed give that $\can : Q\to\tangent{Q}{0}$ is an equivalence.
\end{proof}
...
\begin{proof}
  By \cite[Theorem 7.3.6]{draft} we have $H^1(L,M)=0$, since $L$, like any linearly adequate $R$-module, is an affine scheme. Indeed, it is the kernel of a linear map $f : R^m \to R^n$, and therefore the set of roots of a linear equation system.
...
\]
for all finitely adequate module $C$.
...
\begin{proof}
  ...
  Thus, $Tf = Tg$ since $T\eta_X$ is an $T\eta_X$ is an isomorphism.
