...
\begin{corollary}\label{cor:lin-adeq-fp-thick}
...
\end{corollary}
\rednote{The proof shouldn't be within the statement of the Corollary.}
...
\begin{proof}
  ...
  It now follows that $M$ is linearly adequate by \Cref{cor:lin-adeq-fp-thick}.
\end{proof}
...
To construct this structure, we first establish that $\finadeq$ is closed under taking internal Homs—that is, the module of homomorphisms between any two finitely adequate modules is itself finitely adequate.
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  Then we can reuse the same argument to show the statement of the proposition. \rednote{I guess what is meant is the dual of the argument?}
\end{proof}

Having established that $\Hom_R(-,-)$ serves as an internal Hom for $\finadeq$, it remains to find its left adjoint monoidal product.
In general, the ordinary tensor product $M \otimes N$ of linearly (or finitely) adequate modules fails to be finitely adequate (see \Cref{cex:tensor-not-closed}).
This contrasts with the case where $M$ is finitely presented (or, similarly, where $N$ is finitely presented), since $M \otimes N$ can then be realized as the cokernel of a map between finite direct sums of $N$, and is therefore finitely adequate.

Somewhat surprisingly, the internal Hom still admits a left adjoint in $\finadeq$.
