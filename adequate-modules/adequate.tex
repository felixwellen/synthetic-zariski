...
\begin{remark}
  ...
  then since $M \otimes (-)$ is right exact and $\Hom_R(M^\ast,-)$ is left exact,
  we deduce that both would be exact functors on the subcategory of finitely adequate modules.
  We will show that $M \otimes (-)$ is not exact, even if $M$ is finitely presented.
  (Note that in this case $\Hom_R(M^\ast,-)$ \emph{is} exact, since $M^\ast$ is a projective module
...
\begin{proof}
  ...
  This is equivalent to showing that $M^\ast$ is projective, i.e.\ we need to show that $H :\equiv \Hom_R(M^\ast,-)$ is exact on the category of finitely adequate modules.
  ...
  Thus we have $\Ext_R^1(M^\ast,A)=0$, since \Cref{identification-hom-tensor-for-fp-modules} implies that $\Hom_R(M^{\ast}, -)$ is exact on the category of finitely presented modules.
\end{proof}
...
\begin{remark}
  ...
  \begin{enumerate}
    ...
    It is however worthwhile to understand that, at least as of now, it is only clear that the evaluations of $\Gamma(X, \mc F \otimes \mc G)$ are jointly injective while those of $\Gamma(X, \mc F) \otimes_{R^X} \Gamma(X, \mc G)$ are jointly surjective.
    \rednote{I rephrased to what I think you meant.  But doesn't it follow trivially that the evaluations of $\Gamma(X, \mc F \otimes \mc G)$ are jointly surjective as well?}
