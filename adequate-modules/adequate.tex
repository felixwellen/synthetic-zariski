...
Thus, proving that $(-\dbldualtensor-)$ is the internal left adjoint reduces to verifying that the canonical evaluation map $\ev_{A \otimes B} : A \otimes B \to A \dbldualtensor B$ induces a natural isomorphism
\[
  \ev_{A \otimes B}^\ast : \Hom_R(A \dbldualtensor B, C) \xrightarrow{\;\simeq\;} \Hom_R(A \otimes B, C)
\]
for all finitely adequate modules $A$, $B$, and $C$.
...
\begin{remark}
  ...
  \begin{enumerate}
    ...
    It is, however, worthwhile to understand that, at least at this point, it is only clear that the evaluations of $\Gamma(X, \mc F \otimes \mc G)$ are jointly injective and jointly surjective.
...
\begin{proof}
  ...
  Because of the square above, it is then enough to verify $k|_{D(a_i)}=0$.
  ...
  Since $X$ is affine and all bundles are weakly quasi-coherent, by \cite[Theorem 7.3.6]{draft} we get an induced exact sequence $0 \to \Gamma(X,\mc K) \to A^n \to \Gamma(X,\mc F) \to 0$ which yields a commutative diagram
  \[
  \begin{tikzcd}
