...
\begin{proof}
  By \cref{prp:monad-factorization}, we see that $f$ factors uniquely through $\ev_{A \otimes B}$ if $A \otimes B$ is $M$-idempotent and $C$ is reflexive.
  The module $C$ is reflexive by \cref{thm:can-dual-iso}, as it is finitely adequate.
  The tensor product $A \otimes B$ is $M$-idempotent because $A \dbldualtensor B = \Hom_R(A \otimes B, R)^\ast \simeq \Hom_R(A, B^\ast)^\ast$, which is reflexive by \cref{thm:can-dual-iso} and \cref{prp:fin-adeq-hom-closure} since $A$ and $B$ are finitely adequate.
\end{proof}
