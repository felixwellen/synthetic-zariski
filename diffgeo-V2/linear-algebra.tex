...
\begin{proof}
  ...
  \]
  So by comparing coefficients as a polynomials in $\lambda$, we have $\sum_{\sigma:\N^{\{1,\dots,n\}}, \sum \sigma \neq 1}a_\sigma x_1^{\sigma(1)}\cdots x_n^{\sigma(n)}=0$,
...
\begin{proof}\footnote{This proof was explained to us by Thierry Coquand.}
  TODO
\end{proof}
...
\begin{proof}
  Surjectivity follows from \Cref{extend-from-kernel}, exactness from \Cref{extend-from-image}.
\end{proof}
