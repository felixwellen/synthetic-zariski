...
\begin{corollary}
  Let $X$ be a scheme, then the tangent bundle $X^{\mathbb{D}(1)}$ is a scheme as well.
\end{corollary}

\begin{proof}
  Finitely co-presented modules are schemes, since they are the common zeros of linear functions on $R^n$.
...
The construction is local and one can show by direct computation, that there is the following explicit descrition:

\begin{remark}
  For affine $X=\Spec R[X_1,\dots,X_n]/(f_1,\dots,f_l)$, the tangentbundle is given by
  \[
    X^{\mathbb{D}(1)}=\Spec R[X_1,\dots,X_n,D_1,\dots,D_n]/(f_1,\dots,f_l,(g_{ij})_{ij})
  \]
  where the $g_{ij}=f_i(D_1,\dots,D_n)\cdot f_j(D_1,\dots,D_n)$.
\end{remark}
