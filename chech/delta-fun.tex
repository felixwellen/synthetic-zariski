...
\begin{lemma}
  ...
  and any morphism of $(i-1)$-truncated $\partial$-functors $f:T^{\leq (i-1)}\to T'^{\leq (i-1)}$, there is a unique\index{$\mathrm{ext}(\chi,\mathcal{S})$}
...
\begin{lemma}
  ...
  \begin{center}
    \begin{tikzcd}
      A\ar[r,"r_\chi"]\ar[d,"\varphi_A"] & R_{\chi}\ar[r]\ar[d,"\varphi_R"] & S_{\chi}\ar[d,"\varphi_S"] \\
...
\begin{proof}[of \cref{lem:extension-welldefined}]
  ...
  \begin{center}
    \begin{tikzcd}
      T^{i-1}(R_\chi)\ar[r]\ar[dr]\ar[dd,"f^{i-1}"] & T^{i-1}(C_\chi)\ar[r]\ar[dr]\ar[dd, shift right=1ex, near start, "f^{i-1}"] & T^i(A)\ar[r]\ar[dr,"\varphi^\ast"] & T^i(R_\chi)\dots\ar[dr] & \\
      & T^{i-1}(R_\xi)\ar[r]\ar[dd,"f^{i-1}",crossing over,near start] & T^{i-1}(C_\xi)\ar[r]\ar[dd,crossing over, near start, "f^{i-1}"] & T^i(A')\ar[r] & T^i(R_\xi)\dots \\
