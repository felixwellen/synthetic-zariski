...
\end{itemize}
Then any type that has a Zariski cover by $P$-local pieces is $P$-local.
\end{lemma}

\begin{proof}
First identity type in such an $X$ are locally $P$-local, i.e. are of the form $\Sigma_US$ with $U$ open and $S$ $P$-local. Since open propositions are $P$-local, we get that $X$ is $P$ separated

Now we check that for all such type $X$, any map:
\[f:P\to X\]
merely factors through $1$. Take $(U_i)_{i:I}$ a Zariski cover of $X$ by $P$-local pieces. Then for any $i:I$ we have that $f^{-1}(U_i)$ is an in $P$, so since the type of open is $P$-local, we merely have an open proposition $V_i$ such that for all $x:P$, we have:
...
\[ \lor_{i:I} V_i\]
Assume $k:I$ such that $V_k$ holds. Then $f^{-1}(U_k) = P$ and the map $f$ factors through a $P$-local piece $U_k$. So we merely have a lift for $f$.
\end{proof}
...
\begin{itemize}
\item The type $R$ is an \'etale sheaf by \Cref{etale-subcanonical}, so that any affine scheme is $P$-local.
