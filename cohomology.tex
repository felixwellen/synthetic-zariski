...
It is, however, not jostified to expect, that this application
...
We will need the following algbraic lemma:
...
The equivalence between the two statements in the definition holds because of the
construction of the canonical map in \cref{localization-to-restriction}.
...
\begin{definition}[using \axiomref{sqc}, \axiomref{loc}, \axiomref{Z-choice}]%
  ...
  \item For all open affine $U=\Spec A\subseteq X$ and $f:A$
    ...
    \]
    is an equivalence.
  \item For all $x:X$ and $f:R$, the canonical map from \cref{localization-to-module-if-non-zero} is an equivalence:
    \[
      (M_x)_f\simeq M_x^{f\neq 0}
      \rlap{.}
    \]
