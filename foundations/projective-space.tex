...
\begin{proof}
  ...
  so we have $\A^{\bP^1} = R$ as desired.
\end{proof}
...
\begin{theorem}[using \axiomref{loc}, \axiomref{sqc}]%
  ...
  that is,
  \[ H^0(\bP^n, \A) \colonequiv (\bP^n \to \A) = R \rlap{.} \]
\end{theorem}
...
\begin{remark}
  ...
  for example
  $(U_0 \to \A) = R[\frac{X_1}{X_0}, \dots, \frac{X_n}{X_0}]$.
...
\begin{definition}
  ...
  \item The \notion{dual}\index{$\mathcal L^\vee$} $\mathcal L^\vee$ of a line bundle $\mathcal L:\bP^n\to \Mod{R}$,
    ...
    \[ (x:\bP^n)\mapsto \Hom_{\Mod{R}}(\mathcal L_x,\A)\rlap{.}\]
...
\begin{proof}
  ...
  since we merely have $\mathcal{L}_x = R$ as $\A$ modules.
