...
\begin{definition}%
  ...
  \begin{enumerate}[(a)]
  \item A \notion{line} in an $R$-vector space $V$ is a subtype $L:V\to \Prop$,
    such that there merely is an $x:V\setminus\{0\}$ with
...
\begin{lemma}[using \axiomref{loc}, \axiomref{sqc}]%
  ...
  The inclusion of constant functions into $\bP^1 \to R$ is an equivalence.
\end{lemma}
...
\begin{proof}
  ...
  with inclusion of constant polynomials as projections.
  This shows that the inclusion of constant functions $R\simeq R^{\bP^1} $ is an equivalence.
\end{proof}

\begin{lemma}[using \axiomref{loc}, \axiomref{sqc}]%
  \label{parametrized-line-through-two-points-in-projective-space}
  Let $p \neq q \in \bP^n$ be given.
  Then there merely is a map $f : \bP^1 \to \bP^n$
...
\begin{theorem}[using \axiomref{loc}, \axiomref{sqc}]%
  ...
  that is, the inclusion of constants
  \[ R \to  (\bP^n \to R) \eqqcolon H^0(\bP^n, R) \]
  is an equivalence.
\end{theorem}
...
\begin{remark}
  ...
  Then the limit can be computed to be the constant functions.
\end{remark}
...
\begin{definition}%
  ...
  we mean that $\mathcal{L}$ is merely equal to the trivial line bundle,
...
\begin{proof}
  ...
  and a gluing function $g:(x:\A^1\setminus\{0\})\to \mathcal L_0(x)=\mathcal L_1(\frac{1}{x})$.
  Since we are proving a double negation, we can assume identifications $p_0:(x:\A^1)\to \mathcal L_0=R^1$
  and $p_1:(x:\A^1)\to \mathcal L_1=R^1$ by \Cref{line-bundle-A1-notnot-trivial}.
  ...
  and therefore by \Cref{invertible-laurent-polynomials} given by $\alpha X^n$ for some $\alpha:R^\times$ and $n:\Z$.
  ...
  By explicit calculation, the tautological bundle $\mathcal O_{\bP^1}(-1)$ on $\bP^1$ is given by gluing trivial line bundles along
  a gluing function $g_{-1}:(x:\A^1\setminus\{0\})\to R^1=R^1$ with $g_{-1}(x)\colonequiv \lambda\mapsto x\cdot \lambda$.
  Note that an arbitrary choice of sign is involved, made by choosing the direction of the gluing function.
  Sticking with the same choice, calculation shows $g_1(x)\colonequiv \lambda\mapsto \frac{1}{x}\cdot \lambda$
  is a gluing function for the dual of the tautological bundle $\mathcal O_{\bP^1}(1)$
