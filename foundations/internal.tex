...
the proposition $D(\vec{r}) = D(r_1)\vee\dots\vee D(r_n)$.%For $n=0$, we get the proposition $1=_R 0$.
...
%%     \end{corollary}
    ...
    For an f.p.\ $R$-algebra $A$ \rednote{?}, we can define the type of presentations $Pr_{n,m}(A)$ as the type $A[X_1,\dots,X_n]^m$. Each element in $Pr_{n,m}(A)$ defines an
    f.p.\ $A$-algebra. Since $Pr_{n,m}(A)$ is a modal type if $A$ is modal \rednote{I think we only know that, if A is fp R-algebra}, the type of presentations $Pr_{n,m}(A)_S$ in the sheaf model
    ...
    \rednote{Maybe Pr should be defined closer to corollary 8.1.7.}
    ...
    \begin{proof}
      This follows from \Cref{norm} and Lemmas \ref{Um} and \ref{nats}.
    \end{proof}
    ...
    \begin{proof}
      Using the local-global principles presented in \cite{lombardi-quitte}, \rednote{<- this is probably for localizations $A_{r_i}$ - why does it apply?} we can patch together the f.p.\ $R_{r_i}$-algebra
    ...
    as the type stating that there is some map $\alpha:R[X_1,\dots,X_n]\rightarrow A$ and that $(A,\alpha)$ is universal \rednote{among f.p.\ $R$-algebras} such that
    ...
    some $v:Pr_{n,m}(A)$ among the modal \rednote{f.p.\ }$R$-algebras. This is actually equivalent to $\Phi(n,m,A)$ since any f.p. $R$-algebra is modal.
