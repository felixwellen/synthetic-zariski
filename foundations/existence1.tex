...
A context is interpreted as a presheaf valued in $\WW$ over $\CC$. We interpret $\Type(\Gamma)$ as the collection of
...
\begin{proof}
  ...
  Also $u$ is in $L_{\alpha}$ if we have a covering $\alpha_i:B\righatrrow B_{b_i}$ and each $\alpha_i(u)$ is in $L_{\alpha_i\alpha}$.
  ...
  This implies \cite{lombardi-quitte} that $L_{\alpha}$ is finitely generated. But then we can apply Theorem 5.14 of
