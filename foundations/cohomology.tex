...
\begin{definition}
  ...
  \[
    H^k(X,\mathcal F)\colonequiv \left\propTrunc{\prod_{x:X}K(\mathcal F,k)\right}_0\rlap{.}
  \]
\end{definition}
...
\begin{definition}
  ...
  A \notion{$A$-banded gerbe} on $X$ is a dependent, pointwise connected type $\mathcal G:X\to \mathcal U$,
