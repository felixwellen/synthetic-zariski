...
like it would be the case for a sheaf.
...
\begin{proof}
  ...
  $R^p\to R^q\to M\otimes_A x\to 0$. Using Theorem  \ref{H1-wqc-module-affine-trivial} (one can check that there is no
  ...
  By boundedness and local choice, we get $p,q$ and a finite open covering $D(f_1),\dots,D(f_l)$ of $X$
  and $P_i:A_{f_i}^p\to A_{f_i}^q$ such that, for each $x$ in $D(f_i)$, we have a presentation
  $R^p\to_{P_i(x)} R^q\to M_x\to 0$. Using Theorem  \ref{H1-wqc-module-affine-trivial}
