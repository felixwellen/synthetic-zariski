...
\[ \left(\prod_{x : D(f)} R\right) = (D(f) \to \A) = A[f^{-1}] \]
...
\begin{definition}
  ...
  \item We write:
    \[
      ...
      \rlap{.}
    \]
  \item With pointwise structure, $U\to R$ is an $\A$-algebra
...
\begin{theorem}%
  ...
  let a bundle of finitely presented $\A$-modules $M : X\to \fpMod{R}$ be given.
  Then the $A$-module
  \[ \tilde{M}\coloneqq\prod_{x:X}M_x \]
  is finitely presented and for any $x:X$ the $\A$-module $\tilde{M}\otimes_A R$ is $M_x$.
