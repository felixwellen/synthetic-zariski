...
In \Cref{sec:cset-hset-quotient-vs-meta-quotient}, we prove some precise statements linking hset quotients in cubical sets, and quotients in the meta theory.
...
\begin{proof}
  ...
  Since \(D(PA)\) is levelwise equivalent to proposition and \(D\)-modal, it is a proposition by \Cref{prop:psh:levelwise-n-type-if-n-type}~\ref{prop:psh:levelwise-n-type-if-n-type:modal-converse}.
...
This fact is needed for showing that hset quotients are levelwise hset quotients, the key lemma needed in \Cref{sec:k-duality-implies-k-alg-choice}.
...
\begin{proof}
  ...
  \item[\ref{prop:set-quotient-is-hset-quotient-iff-surjection-splits:characterization}] % we can reduce to I = [0] if every object in \square admits a map from [0].
...
\end{lemma}
\begin{proof}
  By \Cref{prop:set-quotient-is-hset-quotient-iff-surjection-splits}, the map \(\Delta S \to \Delta(S/\trunc*{R})\) is a hset quotient of \(\Delta S\) by \(\Delta\trunc*{R}\) exactly if \(S \to S/\trunc*{R}\) splits.
...
\todo[inline]{The TODO above seems wrong. We cannot assume that the section lands in \(P + P\). This is not an issue, since we have weaker taboos we can appeal to. For example, \(\mathbb{F}_2[\Sigma^*] \to \mathbb{F}[\Sigma^*/\sim]\) splitting means that we can decide equality of words modulo rewriting, which is equivalent to deciding the halting problem, and therefore Markovs principle.}
...
\begin{proof}
  ...
  % The latter also implies by effectiveness of the hset quotient that \(\trunc{R_{(n,m,p)}} \leftrightarrow \trunc*{R_{(n,m,p)}}\).

  By \Cref{prop:psh:levelwise-quotient-if-hset-quotient}, we have that the evaluation at \(k \in \Alg*[k]\) yields a hset quotient in \(\cSet\). 
  ...
  \Cref{prop:set-relevant-quotient-is-hset-quotient-iff-surjection-splits-and-witnesses-exist} implies that f.p. \(k\)-algebras have choice and witnesses.
\end{proof}
...
\Cref{prop:duality-for-k-implies-k-choice-and-witnesses} shows that in this setup, we can only expect to derive a duality axiom for base rings \(k\) with choice and witnesses.
...
\begin{proof}
  ...
  Hence, we can deduce as in the proof of \Cref{prop:duality-for-k-implies-k-choice-and-witnesses} that duality holds exactly if for all presentations \((n, m, p) \colon \Delta\Pr\) we have that the following canonical map is a hset quotient of \(\Delta \Term_\R(m)\) by the relation \(\trunc{R_{(n,m,p)}}\) induced by the presentation.
  \[\begin{tikzcd}
  ...
  As in \Cref{prop:duality-for-k-implies-k-choice-and-witnesses}, the case for \(M = k\) implies that f.p. \(k\)-algebras have choice and witnesses.
  
  Conversely, by \Cref{prop:psh:levelwise-quotient-if-hset-quotient}~\ref{prop:psh:levelwise-quotient-if-hset-quotient:modal-converse} it suffices to show for all \(M \in \Alg[k]\) that \(\Delta \paren{\sfrac{M[X_1, \ldots, X_m]}{p_1, \ldots, p_n}}\) is a hset quotient of \(\Delta\Term_{M}(X_1, \ldots, X_n)\) by the relation \(\Delta R_{(n,m,p)}\) in a context with a finite \(M\)-algebra presentation \((n,m,p) \colon \Delta \Pr_M\).
  By \Cref{prop:set-relevant-quotient-is-hset-quotient-iff-surjection-splits-and-witnesses-exist} this is the case if \(M\)-algebras have choice and witnesses, which follows from \Cref{rem:choice-and-witness-property-descend}.
\end{proof}
...
\todo[inline]{We should explain how the relation lifts. Its induction principle is dependent, so it is less clear that it lifts. I think we have to use that paths in \(\R\) are actual equality.}
