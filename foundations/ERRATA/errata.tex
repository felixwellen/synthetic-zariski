...
We freely make us of the interpretation of extensional type theory in presheaf categories~\cite{hofmann,huber-phd-thesis}.
...
\begin{proof}
  ...
  \end{tikzcd}\]
  The definition of the spectrum in \([\Alg*[k], \Set]\) and \([\Alg*[k], \cSet]\) agree, since for each type of the form \(\Delta X\) the diagonal \((\Delta X) \to (\Delta X)^{\mathsf I}\) is an isomorphism since it is levelwise an isomorphism in \(\cSet\).
...
\subsection{The strictification modality}
...
\end{theorem}
\begin{proof}
  \todo[inline]{Forward as before as soon as we observe that \(\Delta\) lands in \(D\)-modal types (just state this); backward we need to work in an arbitrary context/in a context with a presentation.}
  \begin{description}
  \item[``\(\implies\)'']
  \item[``\(\impliedby\)'']
