...
\begin{remark}\label{rem:choice-and-witness-property-descend}
  ...
  We obtain a derivation that \(\set{p_\ell \doteq 0}_\ell, \set{q_\ell \doteq 0}_\ell \vdash c_Mt_0 \doteq c_M t_1\).
  From such a derivation we obtain one \(\set{q_\ell \doteq 0}_\ell \vdash \underline{c_Mt_0} \doteq \underline{c_Mt_1}\) since the additional equations hold in the theory of \(M\)-algebras.
  From \(\set{q_\ell \doteq 0}_\ell \vdash \underline{c_Mt_\ell} \doteq t_\ell\) for \(\ell \in \set{0,1}\) we obtain the final derivation.
\end{remark}
\todo[inline]{The above is a by handwavy; recheck that the transformation of derivations is possible}
...
\begin{proof}
  ...
  \todo[inline]{We need that the good properties of \(k\) are inherited by f.p. \(k\)-algebras (or strengthen the assumption)}
\end{proof}
