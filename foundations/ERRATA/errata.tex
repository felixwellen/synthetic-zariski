...
\begin{proof}
  ...
  \item[\ref{prop:set-quotient-is-homotopy-quotient-iff-surjection-splits:characterization}] % we can reduce to I = [0] if every object in \square admits a map from [0].
    ...
    \]
    % This type can be described in terms of the nerve of a fibration fibered in groupoid over the discrete groupoid \(\pi_0\Gamma\) where the fiber over \([\gamma] \in \Gamma\) is the groupoid with objects \(S[\gamma]\) and exactly one morphism between related elements.
    ...
    \todo[inline]{Are we choosing reps here? I think we only apply this in the empty context, so we can get away with the simpler version}
  \end{description}
\end{proof}
...
\todo[inline]{We have to argue that the objects defined above are fibrant and $D$-modal}
