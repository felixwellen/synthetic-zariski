...
\begin{definition}%
  A type $X$ is \notion{(qc-)affine},
  if there merely is a finitely presented $R$-algebra $A$, such that $X=\Spec A$.
\end{definition}
...
\begin{example}[using \axiomref{loc}, \axiomref{sqc}]
  ...
  \[ D((X - a_1) \cdots (X - a_n)) = \A^1 \setminus \{ a_1, \dots, a_n \} \colonequiv \sum_{x:\A^1}\forall i. x\neq a_i\rlap{.}\]
  Indeed,
  for any $x : \A^1$,
...
Algebraically, such a partition is given by a finite system of orthogonal idempotents:

\begin{corollary}[using \axiomref{loc}, \axiomref{sqc}]%
  Let $f:\Spec(A)\to\N$.
...
\begin{proof}
  By \Cref{boundedness} we merely have a bound $N:\N$ for $f$ and we let $e_i$ be the indicator function of $f^{-1}(i)$.
\end{proof}
