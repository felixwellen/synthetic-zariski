...
\begin{lemma}
  Let $I$, $X$ be types, $U_i:X\to\Prop$ a subtype for any $i:I$ and $V$ a subtype of $X$.
  \begin{enumerate}[(a)]
  \item Any subtype $P:V\to\Prop$ is a subtype of $X$ given by $(x:X)\mapsto\sum_{p:V(x)}P((x,p))$.
...
\begin{definition}%
  ...
  A family of subtypes $U_i:X\to\Prop$ \notion{covers} $X$,
...
\begin{lemma}%
  \label{kraus-glueing}
  Let $X$ and $I$ be types.
  For subtypes $(U_i:X \to \Prop)_{i:I}$ that cover $X$ and $P:X\to \nType{0}$, we have the following gluing property: \\
...
\begin{lemma}%
  ...
  Let $X$ and $I$ be types.
  For subtypes $(U_i:X \to \Prop)_{i:I}$ that cover $X$ 
  and $P:X\to \nType{1}$, we have the following gluing property: \\
  If for each $i:I$ there is a dependent function $s_i:(x:U_i)\to P(x)$ together with
...
\begin{definition}
  ...
  i.e. a ring $A_f$ together with a homomorphism $A\to A_f$ such that the image of \(f\) in \(A_f\) is invertible satisfying the following universal property: For all homomorphisms $\varphi:A\to B$ such that
