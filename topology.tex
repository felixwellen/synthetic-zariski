...
We now show that our two definitions (\cref{def:affine-open}, \cref{def:qc-open})
...
\begin{lemma}[using \axiomref{loc},\axiomref{sqc},\axiomref{Z-choice}]%
  ...
  \begin{enumerate}[(a)]
  \item qc-open propositions are closed under sums.
...
\begin{proof}
  \begin{enumerate}[(a)]
  \item Apply \cref{lem:qc-open-trans} to the point $\Spec R$.
...
\Cref{thm:boundedness} can be read as a compactness property
...
\begin{proof}
  ...
  Using \cref{lem:qc-open-sigma-closed}, $V\subseteq X$ is open and $V\cap A=V\cap (U_i\cap A)=\emptyset$.
  So $V=\emptyset$.
\end{proof}
...
\begin{proposition}
  Any regular (\cref{def:regular-element}) element $r:A$ is nilregular.
\end{proposition}

\begin{lemma}[using \axiomref{sqc},\axiomref{loc},\axiomref{Z-choice}]%
  \label{lem:affine-open-dense-nilregular}
...
\begin{proof}
  ...
  By \cref{thm:qc-open-affine-open}, there are $r_1,\dots,r_n:A$ such that $U=D(r_1,\dots,r_n)$.
  Let $x:A$ such that all $r_ix$ are nilpotent.
  By \cref{lem:standard-open-empty}, this implies $D(r_ix)=\emptyset$ for all $i$.
...
\begin{proof}
  By \cref{lem:affine-open-dense-nilregular} and \cref{lem:dense-local}.
\end{proof}
