...
\begin{remark}\label{StoneClosedUnderPullback}
  \rednote{We need this after having introduced $\ODisc$, if we can quickly prove that $\Boole$ are exactly 
  the Boolean algebras in $\ODisc$, the pushout result follows from sequential colimits commuting with finite colimits.}
...
%  finitely cocomplete category (\Cref{CoCompletenessBoole}), the category of Stone spaces is complete. 
  One can show that countably presented Boolean algebras are closed under pushouts. 
  By \Cref {SpIsAntiEquivalence} %and the fact that that countably presented Boolean algebras are closed under pushouts, 
  it follows that the category of Stone spaces is closed under pullbacks. 
\end{remark}
