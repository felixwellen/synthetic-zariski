...
%\begin{proof}
%  Let $A:\N \to \Open$. 
%  By countable choice, there exists a map $\alpha_{(\cdot)}:\N \to 2^\N$ such that 
...
%  $A(n) \leftrightarrow \exists_{m:\N} \alpha_n (m) = 1$. 
%  Define $B:\N \times \N \to 2$ by $B(m,n) = \alpha_n(m)$. 
%  Then $A(n) \leftrightarrow \Sigma_{m:\N} B(m,n) $. 
%   NOT SUFFICIENT
%\end{proof}
