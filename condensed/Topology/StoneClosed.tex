...
%      $$Sp(C) = \{x:S| \forall_{n:\N} x(\neg d_n) =0\}  = \bigcap_{n:\N} D_n.$$
      By \Cref{AxStoneDuality}, we have $d_n,~n:\N$ terms of $B$ such that $D_n = \{x:S| x(d_n) = 0\}$. 
      Let $C = B/(d_n)_{n:\N}$.
      Then the map $Sp(C) \to S$ is as desired because
...
%      as we untruncated our assumptions before we specified $x$. 
   ...
   \item[$(v) \to (iv)$.]
     ...
     By \Cref{AxLocalChoice} gives us that there merely exists $T, e, \beta_\cdot$ as follows:
     \begin{equation}
       \begin{tikzcd}
...
\end{lemma}
\begin{proof}
  \rednote{Too shorten this (and some other proofs), I've removed some negations and pretended  $D:S\to 2$ is given by 
  $\{x:S|x(d) = 0\}$ instead of $\{x:S|x(d) = 1\}$ }
  Assume $S = Sp(B)$. 
  By \Cref{StoneClosedSubsets}, there exists sequences $f_n,g_n:B,~n:\N$ such that 
