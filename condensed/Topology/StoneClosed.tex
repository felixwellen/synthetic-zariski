...
\item[$(iii)\to (i)$] By we have that $Sp(A) = \cap_{n:\N}D_n$ for $D_n$ decidable in $Sp(B)$. Assuming $D_n$ correponds to $b_n:B$ though duality, we then have that $A=B/\langle (b_n)_{n:\N} \rangle$ and the induced map is the quotient map:
$$B\to B/\langle (b_n)_{n:\N} \rangle$$
which is surjective.
\end{itemize}
\end{proof}
