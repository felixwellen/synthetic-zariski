...
%\begin{lemma}
%  Let $X:\Chaus, x:X$, and let $A,B\subseteq X$ be disjoint and both closed.  
%  If $Q_x = A \cup B $, then $\neg A$ or $\neg B$.
%\end{lemma}
%\begin{proof}
%  Let $q:S \to X$ be the quotient map of a presentation, 
%  by \Cref{StoneSeperated}, there is some $D\subseteq S$ decidable with 
%  $q^{-1}(A) \subseteq D, q^{-1}(B) \subseteq \neg D$. 
%  Hence $A\subseteq q(D), B\subseteq q(\neg D)$. Furthermore, $q(D), q(\neg D)$ are closed. 
%\end{proof}
