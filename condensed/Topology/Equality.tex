...
\begin{proof}
  ...
  By \Cref{AxStoneDuality}, we must have $Sp(2^{S_n}) = S_n$, hence $Sp(2^S) = S$, 
  and $S$ is Stone. 
\end{proof}
\begin{rmk}
  Whenever $S:\Stone$, we shall 
\end{rmk}
...
\begin{lemma}\label{scott-continuity}
  Suppose $Z$ is a type with a surjection $e:\N \twoheadrightarrow Z$, 
  and let $S_n$ be a $(\N,\geq)$-indexed sequence with limit $S$. 
%  $S:\Stone$ be the limit of the $(\N,\geq)$-indexed sequence $S_n$. 
  Then $Z^S$ is the colimit of the $(\N,\leq)$-indexed sequence $Z^{S_n}$.
