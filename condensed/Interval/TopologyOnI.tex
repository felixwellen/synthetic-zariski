...
%  The proof is similar to $\alpha'\leq_I\gamma'$ and $\gamma'\sim_I\beta'$, we have $\alpha'\leq_I\beta'$.
  ...
  As $\beta\leq_I\gamma$ is closed, by \Cref{rmkOpenClosedNegation} we will show it's double negation instead. 
  ...
  Yet from $\alpha\sim_I\beta$, using \Cref{alternativeSimByCauchyDistance} and \Cref{CauchyApproxLemma}, 
  for $n\geq N$ we get 
...
\end{proof}
%\begin{lemma}
%  Whenever $\alpha\leq_I\gamma$ and $\beta\sim_I\gamma$, we have 
%  $\alpha\leq_I\beta$. 
%\end{lemma}
%\begin{proof}
%  As $\alpha\leq_I\beta$ is closed, we will show it's double negation instead. 
%  By Markov, the negation is that there is some $N:\N$ with 
%  $cs(\alpha)_N > cs(\beta)_N + \frac{1}{2^N}$. 
%  As $\alpha\leq_I\gamma$, 
%  we still have 
%  $cs(\gamma)_N + \frac{1}{2^N} \geq cs(\alpha)_N$. 
%  It follows that $cs(\gamma)_N > cs(\beta)_N$. 
%  As $\beta\sim_I\gamma$, it follows from \Cref{alternativeSimByCauchyDistance} 
%  and \Cref{CauchyApproxLemma} that 
%  $cs(\gamma)_n = cs(\beta)_n + \frac{1}{2^n}$ for $n\geq N$. 
%  But then we can conclude 
%  $$
%  cs(\gamma)_N + \frac{1}{2^N} \geq cs(\alpha)_N > cs(\gamma)_N
%  $$
%  and again we have $cs(\alpha)_N = cs(\gamma)_N + \frac{1}{2^N}$. 
%  and as $\alpha\leq_I\gamma$, we again have that $\alpha\sim_I \gamma$, and by same
%\end{proof}
