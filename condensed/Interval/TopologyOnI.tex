...
%  The proof is similar to $\alpha'\leq_I\gamma'$ and $\gamma'\sim_I\beta'$, we have $\alpha'\leq_I\beta'$.
  ...
  As $\beta\leq_I\gamma$ is closed, by \Cref{rmkOpenClosedNegation} it is double negation stable. 
...
\rednote{These properties are nice but not necessary:}
\begin{corollary}
  We have the following inequalities for $x,y:I$:
  \begin{enumerate}
...
\begin{proof}
  \begin{enumerate}
    \item As a consequence of $((x\leq y) \wedge (y \leq x )) \leftrightarrow (x = y)$, 
        if $x\leq y \wedge x \neq y$, we have $\neg (y \leq x)$, hence $x<y$. 
        As a consequence of the same, if $x<y$ we must have $x\leq y$, ans $y\leq x $ is impossible. 
        It's immediate that $x<y$ implies $x\neq y$. 
    \item Follows directly from the previous item and \Cref{IntervalOrderLeqOrGeq}
    \item 
  \end{enumerate}
\end{proof}
