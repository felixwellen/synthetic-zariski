...
We omit the proof that $\mathbb{S}^1\colonequiv\{(x,y):\R\vert x^2+y^2=1\}$ is equivalent to $\R/\Z$ (TODO reference for why this is good).
...
\begin{theorem}[Brouwer's fixed-point theorem]
  Any $f:\mathbb{D}^2\to \mathbb{D}^2$ has a fixed-point, i.e.\ there exists $x:\mathbb{D}^2$ such that $f(x)=x$.
\end{theorem}

\begin{proof}
  For a surjection $\pi:S\to \mathbb{D}^2$ from a Stone space $S$, the proposition $\exists x:\mathbb{D}^2.f(x)=x$ is equivalent to  $\exists y:S.f(\pi(y))=\pi(y)$ and therefore closed.
  Since closed propositions are $\neg\neg$-stable, we can conclude with a proof by contradiction.
  So we assume $\neg \exists x:\mathbb{D}^2. f(x)=x$ which implies that we have $f(x)\neq x$ for all $x:\mathbb{D}^2$.
  For any $x:\mathbb{D}^2$ we set $d_x\colonequiv x-f(x)$, so we have that one of the coordinates of $d_x$ is invertible.
  Let $r(x)$ be the intersection of the half-line $(t:\R_{>0}\mapsto f(x) + t\cdot d_x$, then $r$ is a retraction from $\mathbb{D}^2$ onto its boundary $\mathbb{S}^1$, which is a contradiction by \cref{no-retraction}.
  TODO: Argument why there is a intersection.
\end{proof}
