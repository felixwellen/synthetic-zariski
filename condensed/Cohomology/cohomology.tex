...
We have that $(C_n,\sim_n)$ is equivalent to $\mathrm{Fin}(2^n)$ with $\lambda x,y. |x-y|\leq 1$.
\end{lemma}
...
\begin{proof}
It is clear that the map $\Z\to \Z^{C_n}$ is injective as $C_n$ is non-empty, so the sequence is exact at $\Z$.
...
Assume a cocycle $\beta:\Z^{C_n^{\sim3}}$, meaning that for all $u,v,w:C_n$ such that $u\sim_nv$, $v\sim_nw$ and $u\sim_nw$ we have that:
...
\begin{proof}
Consider the \v{C}ech cover of $\mathbb{I}$ by: 
\[T_x = \sum_{y:2^\N} x=_\mathbb{I} [y]\]
Then we have that:
