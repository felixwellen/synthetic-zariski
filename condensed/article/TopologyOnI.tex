...
\begin{definition}
  Let $x,y:\I$ and $\alpha,\beta:2^\N$ such that $x=cs(\alpha), y=cs(\beta)$.
  Then $x<y$ is the proposition $\exists_n.cs_n(\alpha)+\frac{1}{2^{n}}<_{\mathbb Q}cs_n(\beta)$, which is independent of the choice of $\alpha,\beta$.
\end{definition}

\begin{remark}\label{LesserOpenPropAndApartness}
  Let $x,y:\I$. Then $x<y$ is an open proposition and we have that $x\neq y$ is equivalent to $(x<y) \vee (y<x)$.
\end{remark}
%
