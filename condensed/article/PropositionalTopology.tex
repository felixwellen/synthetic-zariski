...
\end{remark}
\rednote{Phrase the following lemmas as one big lemma, 
and use them silently without reference, also we should just state $\neg\neg$-stability instead of referring to the above all the time. }
...
\end{lemma}
%\begin{proof}
%We use \Cref{ClosedFiniteDisjunction} and the fact that $\neg(P\lor Q) \leftrightarrow \neg P \land \neg Q$.
%\end{proof}
%\begin{proof}
...
\end{lemma}
\begin{proof}
  By \Cref{ClosedFiniteDisjunction}, $\neg P \vee Q$ is closed, hence $\neg\neg$-stable 
