...
\begin{proof}
  ...
  Sonce $2^{\mathrm{Fin}(k)}$ is finite, we have that $\Hom(2^{\mathrm{Fin}(k)},\_)$ commutes with sequential colimits, therefore $\Hom(2^{\mathrm{Fin}(k)},2^S)$ is the colimit of $\Hom(2^{\mathrm{Fin}(k)},2^{S_n})$. 
  By applying \Cref{SpIsAntiEquivalence} again, these types are $\mathrm{Fin}(k)^{S_n}$ as required. 
\end{proof}
