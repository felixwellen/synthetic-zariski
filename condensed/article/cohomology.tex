...
\begin{remark}
We write $Ab$ for the type of Abelian groups and if $G:Ab$ we write $BG$ for the delooping of $G$ \cite{hott,davidw23}. This means that $H^1(X,G)$ is the set truncation of $X \to BG$. 
\end{remark}
...
Assume given $S:\Type$ with $T:S\to \Type$ and $A:S\to\mathrm{Ab}$ with $t:\prod_{x:S}T_x$. Then $\check{C}(S,T,A)$ is exact.
\end{lemma}
...
Assume given $S:\Type$ with $T:S\to \Type$ such that $\prod_{x:S}\propTrunc{T_x}$ and $A:S\to\mathrm{Ab}$ such that $\check{C}(S,T,A)$ is exact.
Then given $\alpha:\prod_{x:S}BA_x$ with $\beta:\prod_{x:S} (\alpha(x) = *)^{T_x}$, we can conclude $\alpha = *$.
\end{lemma}
