...
Using \cref{description-Cn-simn} to pass along the equivalence between $2^n$ and $\mathrm{Fin}(2^n)$ we can define $\alpha(k) = \beta(0,1)+\cdots+\beta(k-1,k)$.
...
\begin{remark}
In constructive reverse mathematics \cite{HannesDiener}, it is known that both the intermediate value theorem and Brouwer's fixed-point theorem are equivalent to LLPO.
\end{remark}
