...
%  In particular for $q:S\to X$ the quotient map, $q^{-1}(A)$ is closed. 
  As $p$ is surjective, we have $p(p^{-1}(A)) = A$.
...
%  Hence by 
  ...
  As $B(t)$ and $p(s) = p(t)$ are closed, by \Cref{ClosedCountableConjunction} and \Cref{InhabitedClosedSubSpaceClosed}, 
...
%  The characterization of open sets 
  The rest then follows from \Cref{rmkOpenClosedNegation} and \Cref{ClosedMarkov}.
\end{proof}
%
\begin{remark}
