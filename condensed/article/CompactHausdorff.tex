\begin{definition}
  A type $X$ is called a compact Hausdorff space if its identity types are closed propositions and there exists some $S:\Stone$ and a surjection $S\to X$.
\end{definition}
...
%  In particular for $q:S\to X$ the quotient map, $q^{-1}(A)$ is closed. 
  As $q$ is surjective, we have $p(p^{-1}(A)) = A$.
  If $A$ is closed, so is $p^{-1}(A)$ and 
...
%  Hence by 
  ...
  $$A'(s) := \exists_{t:S} (B(t) \wedge p(s) = p(t)).$$
  As $B$ and $p(s) = p(t)$ are closed, by \Cref{ClosedCountableConjunction} and \Cref{InhabitedClosedSubSpaceClosed}, 
  we have that $A'$ is closed. 
  Also $A'$ factors through $p$ as a map $A: X\to \Closed$.
  Furthermore, $A'(s) \leftrightarrow (p(s)\in p(B))$. 
  Hence $A=p(B)$. 
