...
\begin{proof}
  \begin{itemize}
    ...
    \item 
      ...
      and their conjunction is closed by the above. 
...
\begin{proof}
  \begin{itemize}
    \item $(i) \to (ii)$.
      ...
      $f(x) = 1$ if $D(x)$ and $f(x) = 0$ if $\neg D(x)$. 
      Stone duality then gives that this term corresponds to evaluation at some $b:B$. 
      Therefore $D(x) \iff f(x) = 1 \iff  x(b) = 1$, as required. 
    \item $(i) \to (iv)$. 
      Follows from the fact that decidable propositions are 
      both open and closed. 
      Let $p$ be decidable. 
      To see $p$ is closed, consider the sequence $\alpha:\Noo$ given by 
      $\alpha(n) = 0$ iff $p$. Now $\alpha =\infty $ iff $p$ as well. 
      To see $p$ is open, consider the sequence $\alpha:\Noo$ given by 
      $\alpha(0) = 1$ iff $p$ and $\alpha(n) = 0$ for $n>0$. 
      Then $\alpha\neq \infty$ iff $p$ holds. 
    \item $(ii) \to (i)$
      Let $D$ be as in $(ii)$. 
      For $y:Sp(B)$, we have that 
      $D(y)$ iff $y(b) = 1$, which is decidable, 
      as $x = 0 \vee x = 1$ for all $x:2$. 
    \item $(ii) \to (iii)$. 
      Suppose there exists a $b:B$ such that $D(x)$ iff $x(b) = 1$. 
      As $b$ is expressable using only finitely many generators $G_0$
      we need to only evaluate $x(g)$ for all $g:G_0$ to 
      calculate $x(b)$. 
      In particular, if $x(g) = y(g)$ for all $g:G_0$, then 
      $x(b) = 1 \leftrightarrow y(b) = 1$. 
      Hence $D(x) \leftrightarrow D(y)$. 
    \item $(iv) \to (i)$. 
      Suppose $D\subseteq S$ is both open and closed. 
      Note that $D(x) \to D(x)$. 
      Here $D(x)$ is both open and closed. 
      By \Cref{OpenClosedImplication}, we thus get 
      $D(x) \vee \neg D(x)$. Thus $D$ is decidable. 
    \item $(iii) \to (i)$  \rednote{TODO}
  \end{itemize}
\end{proof}
\begin{lemma}
