...
\rednote{quick note I forgot so write here but it must go somewhere else}
\begin{axiomNum}[double negation to merely]
  For $S:\Stone$, we have that $\neg \neg S \to || S ||$
\end{axiomNum}
\begin{proof}
  Let $B:\Boole$ and suppose $\neg \neg Sp(B)$, then we have $0\neq 1$ in $B$. 
  Let $f:2 \to B$, note that if $f(0) = f(1)$ then $0=1$ in $B$, thus bottom, thus $0=1$ in $2$. 
...
\begin{proof}
  ...
  Furthermore, we claim that $f$ is injective. 
  Suppose that $f(x) = f(y)$, we make a case distinction on whether $x,y$ correspond 
  to finite or cofinite subsets of $\mathbb N$. 
  \begin{itemize}
    \item Let $x,y$ both correspond to finite subsets of $\mathbb N$. Write 
      $x = \bigvee_{i\in I} p_i, y = \bigvee_{j\in J} p_j$ for $I,J\subseteq \mathbb N$ finite. 
  \end{itemize}
