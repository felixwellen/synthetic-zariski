...
\begin{proof}
  \begin{itemize}
    ...
    \item To see $\sim_I$ is transitive, that 
      ...
      (b \wedge \neg c) \vee (\neg b \wedge c) \in I
      $
      for $I$ an ideal, we must have that for $x,y$ these two terms, 
      we have that $(x \wedge \neg y)\vee (\neg x \wedge y)$ is also in $I$. 
      For $x,y$ as above, 
      note that $\neg y$ is given by 
      $ 
      \neg ((b \wedge \neg c) \vee (\neg b \wedge c) )
...
\begin{remark}\label{rmkMorphismsOutOfQuotient}
  ...
  $\phi[x/f(x)] = 0_B$, 
  where $\phi[x/f(x)]$ replaces every occurence of some $x\in G$ with $f(x)\in B$. 
\end{remark}
\begin{remark}
...
\begin{example}
  ...
  It's underlying set is $\{0\}$ and we have $0=1$ in the trivial Boolean algebra. 
\end{example}
...
\end{lemma}
\begin{proof}
  First note that a Boolean algebra being trivial is a proposition and 
  if we can write $1 = \bigvee_{r \in R_0} r$ in $B$, we have 
  that in $B/R$, $1 = \bigvee_{r \in R_0} 0 = 0$. 

  Conversely, if $1 = 0$ in $B /R$, we must have a finite sequence of relations from $R$ 
  and the theory of Boolean algebras allowing us to rewrite $1$ to $0$. 
\end{proof}
...
%  $\neg \forall_{n:\mathbb N} \alpha(n) = 0 \to \exists_{n:\mathbb N} \alpha(n) = 1$. 
\end{theorem}
\begin{proof}
  ...
  If it is, there exists a finite sequence $(\alpha(n_i) = 0 )_{i\leq k}$ rewriting $1$ to $0$. 




  Note that for each $n\in\mathbb N$, we do have $\alpha(n) = 0 \vee \alpha(n) = 1$, 
  and it cannot be the case that $\alpha(n_i) = 0$ for all $i\leq k$,
  there must 


  Suppose $x: 2 \to 2$ is such that $x(\alpha(n)) = 0$ for every $n:\mathbb N$. 
  as $x(1) = 1\neq 0$, we must have for every $n:\mathbb N$ that $\alpha(n) \neq 1$
  we have that $x(\alpha(n)) = 0$ for all $n:\mathbb N$. 
\end{proof}
