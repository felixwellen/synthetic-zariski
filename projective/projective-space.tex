...
In particular, $R$ denotes the generic local ring and $GL_1$ is the multiplicative group of units of $R$.
...
of $GL_1$. It can be defined as the type of lines $\Sigma_{M:\Mod{R}}\|{M=R^1}\|$.
An equivalent definition of $\bP^n$ is then
$$
\bP^n = \sum_{l:BGL_1}l^{n+1}-\{0\}
$$
...
\cite{Sym}.
Note that we use the same notation for an element $l : BGL_1$ and its underlying set.

The standard line bundles (or twisting sheaves) on $\bP^n$ can then be constructed as follows.
We define $l^{\otimes n}:BGL_1$ for $l:BGL_1$ by
$l^{\otimes 0} = R^1$,
$l^{\otimes (n+1)} = l^{\otimes n}\otimes l$ for $n \geqslant 0$
and $l^{\otimes (n-1)} = l^{\otimes n}\otimes l^{\vee}$ for $n \leqslant 0$.
Then we define the line bundle $O(d):\bP^n\rightarrow BGL_1$ by $O(d)(l,s) = l^{\otimes d}$.
