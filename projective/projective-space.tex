...
\cite{Sym}.
Note that we use the same notation for an element $l : B\Gm$ and its underlying set.
...
\begin{proposition}\label{end}
  $$\prod_{(l,p):B\Gm}l^n\rightarrow l \;\;\;=\;\;\; \Hom(R^n,R)$$
\end{proposition}
...
\begin{proof}
  ...
  $$\prod_{(l,p):B\Gm}l^{n+1}-\{0\}\rightarrow R$$
...
$$\prod_{(l,p):B\Gm}(l^{n+1}-\{0\})\rightarrow (l^{n+1}-\{0\}) \;\;=\;\; GL_{n+1}$$
\end{proposition}
...
$$
is a bijection for $n>0$ if $(l,p):B\Gm$.
...
\end{proof}
 ...
 \begin{lemma}\label{hom}
   An element in
  $$\prod_{(l,p):B\Gm}(l^{n+1}-\{0\})\rightarrow ((l^{\otimes d})^{m+1}-\{0\})$$
