...
There is another definition of $\bP^n$ which uses ``higher'' notions. Let $BR^\times$ be the delooping
of $R^\times$. It can be defined as the type of lines $\Sigma_{M:\Mod{R}}\|{M=R^1}\|$. Over $BR^\times$ we have the
family of sets
$$T_n(l) = l^{n+1}\setminus\{0\}$$
Note that we use the same notation for an element $l : BR^\times$,
its underlying $R$-module and its underlying set.
An equivalent definition of $\bP^n$ is then
$$
\bP^n = \sum_{l:BR^\times}T_n(l)
$$
...
and $l^{\otimes n}:BR^\times$ for $l:BR^\times$ by
$l^{\otimes 0} = R^1$,
$l^{\otimes (n+1)} = l^{\otimes n}\otimes l$ for $n \geqslant 0$
and $l^{\otimes (n-1)} = l^{\otimes n}\otimes l^{\vee}$ for $n \leqslant 0$.
Then we define the line bundle $O(d):\bP^n\rightarrow BR^\times$ by $O(d)(l,s) = l^{\otimes d}$.
...
\begin{proposition}\label{end}
  $$\prod_{l:BR^\times}l^n\rightarrow l \;\;\;=\;\;\; \Hom(R^n,R)$$
\end{proposition}
...
\begin{proposition}\label{aut}
  For all $n:\N$ we have:
$$\prod_{l:BR^\times}T_n(l)\rightarrow T_n(l) \;\;=\;\; GL_{n+1}$$
\end{proposition}
...
\begin{proof}
  ...
  The first remark is that maps $T_n(R)\to T_n(R)$, which are invariant under the induced $BR^\times$ action, are linear.
...
$$
is a bijection for $n>0$ and all $l:BR^\times$.
...
\begin{lemma}\label{hom}
  \begin{enumerate}[(i)]
    \item
      \[  \prod_{l:BR^\times}l^n\rightarrow l^{\otimes d} \;\;=\;\; (R[X_1, \dots, X_n])_d \]
      That is,
      every element of the left-hand side is given by
      a unique homogeneous polynomial of degree $d$ in $n$ variables.
    \item
      An element in
      $$\prod_{l:BR^\times}T_n(l)\rightarrow T_m(l^{\otimes d})$$
