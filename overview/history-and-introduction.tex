...
as Kock and Lawvere did with differential geometry in their work.
...
To give an example, the tangent bundle of a manifold $M$ can be defined as $M^{\D(1)}$ and vector fields as sections of the map $M^{\D(1)}\to M$ given by evaluation at $0$. Then it is easy to see that a vector field is the same as a map $\zeta:\D(1)\to M^M$ with $\zeta(0)=\id_M$, which can be interpreted as an infinitesimal transformation of the identity map. This style of reasoning with spaces as if they were sets is also central in current synthetic algebraic geometry. 

Let us assume that $R$ is non-trivial, i.e.\ that we have $1\neq 0$ in $R$. Then the Kock-Lawvere axiom above is incompatible with the law of excluded middle (LEM) and therefore also with the axiom of choice (AC). Indeed they imply all maps from $R$ to $R$ are differentiable, which contradicts LEM. 
However, restricted versions of LEM and AC are compatible with the Kock-Lawvere axiom.
A very basic example that LEM holds for some propositions in the flavor of synthetic mathematics we study, is that equality of natural numbers is always decidable, meaning that two natural numbers are either equal or not equal. We will latter go back to why full LEM and AC tend to be incompatible with synthetic approach to various geometry.

The use of nilpotent elements to capture infinitesimal quantities as mentioned above was inspired by the Grothendieck school of algebraic geometry. Anders Kock also worked with an extended axiom \cite{Kock74,kockreyes} suitable for synthetic algebraic geometry, where the role of $\D(1)$ above can be taken by any finitely presented affine scheme. In his 2017 doctoral thesis, Ingo Blechschmidt noticed a property holding internally in the Zariski-topos, which he called synthetic quasi-coherence. It is a generalised and fully internal version of what Kock used. In 2018, David Jaz Myers\footnote{Myers' never published on the subject, but communicated his ideas to Felix Cherubini and in talks to a larger audience \cite{myers-talk1,myers-talk2}.} started working with a specialization of Blechschmidt's synthetic quasi-coherence, which is what we now call \emph{duality axiom}.
...
Then the set of roots of the system is given by the type $\Hom_{\Alg{R}}(A,R)$ of $R$-algebra homomorphisms from $A$ to the base ring $R$.
We denote this type of roots by $\Spec A$.
...
Myers used homotopy type theory as a base language, which is now the standard in synthetic algebraic geometry. An introduction to homotopy type theory can be found in \cite{shulman-logic-of-spaces}.
In the next paragraphs we will explain how it fits with synthetic algebraic geometry. Homotopy type theory is a language for synthetic homotopy theory.
...
So the rules of homotopy type theory allow us to work with types in very much the same way as one would work with homotopy types in traditional mathematics. And we will in fact mostly work with homotopy types without homotopical information beyond their connected components which we call \emph{sets} in homotopy type theory. These sets will however still carry spatial structure which can be exposesd through the axiom we introduce below.

A difference to the usual practice of mathematics is, that the law of excluded middle (LEM) or the axiom of choice (AC) are not assumed to hold by default. As indicated above, they are incompatible with the spatial structure of sets in synthetic algebraic geometry. Indeed, LEM allows us to find a complement of each subset $U$ of a given type $A$, which exposes $A$ as a coproduct of $U$ and its complement.
This is not true for spaces. For example, the topological space $\R$ with the euclidean topology is not the coproduct of the topological subspaces $\{0\}$ and $\R\setminus\{0\}$.
AC states that any surjection has a section. This is also not true in general for any sensible notion of space, in particular it would trivialise all cohomology.
Thus, constructive reasoning in the sense of not using LEM and AC is a necessity if we want types to be understood as having a spatial structure. It turns out that apart from this restriction to constructive reasoning, we can work with the spatial objects of algebraic geometry, schemes, synthetically just as if they were sets. 

Working inside homotopy type theory, we can also make use of types that are not sets. We call these types \emph{higher types} and beyond convenience, we can  use them to define cohomology, as we will explain in the next paragraph. This means that we are mixing two synthetic approaches, combining their advantages. This advantage rests on the possibility of interpreting homotopy type theory in any higher topos \cite{shulman2019all} and not just the higher topos of $\infty$-groupoids. More precisely, we think of the higher topos of Zariski sheaves with value in homotopy type. 
...
Schreiber suggested to the homotopy type theory community at various occasions to make use of homotopy type theory as the internal language of higher toposes, where specifities of the topos are accessed in the language via modalities.
This approach was shown to be quite effective and intuitive in Shulman's \cite{shulman-Brouwer-fixed-point} work on mixing synthetic homotopy theory in the form of homotopy type theory and a synthetic approach to topology using a triple of modalities -- a structure called cohesion by Lawvere \cite{Lawvere2007}. 
...
This internal version of cohomology does not agree with the external version mentioned above, indeed it is a sheaf of groups instead of a single group, and it is indexed by an internal natural number instead of an external one. Nevertheless, internal cohomology turned out to be at the very least useful in practice.
...
Below, we will reformulate this axiom using dependent functions.

In total, the system we use for synthetic algebraic geometry consists of the extension of homotopy type theory postulating a fixed commutative ring $R$ satisfying these three axioms -- where the first one just asks the ring $R$ to be a local ring:
...
More surprisingly, it implies that all functions in $R\to R$ are polynomials and that the base ring $R$ has stronger properties than just being a local ring.
...
A \emph{scheme} is a type $X$, that merely has a finite open cover by affine schemes.
