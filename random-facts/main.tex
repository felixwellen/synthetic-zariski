% latexmk -pdf -pvc main.tex
% latexmk -pdf -pvc -interaction=nonstopmode main.tex
\documentclass{../util/zariski}

\title{Random Facts in Synthetic Algebraic Geometry}
\author{???}

\begin{document}

\maketitle

\begin{abstract}
  The following is a collection of results in synthetic algebraic geometry,
  that didn't find a place anywhere else so far.
\end{abstract}

\tableofcontents

\section{Translations of Classical Proofs}
\input{classic.tex}

\section{Homogeneity}
\input{homogeneity.tex}

\section{Serre's affineness theorem}
\input{cohomology-affine.tex}

\section{Line bundles and divisors}
\input{line-bundles.tex}

\section{Segre Embedding}
\input{segre.tex}

\section{Blow-up of Projective Space}
\input{blow-up.tex}

\section{External justification of axioms}
\input{external.tex}

\section{Group schemes are not smooth}
\input{groups-not-smooth}

\section{Hilbert's Nullstellensatz}
\input{nullstellensatz.tex}

\section{Automorphisms of projective space}
\input{aut-pn.tex}

\section{Projective module and vector bundle}
\input{projective-module.tex}

\section{Overtness}
\input{overtness.tex}

\section{$\nabla$-modality}
\input{nabla.tex}

\section{Non-commutative algebraic geometry: Zariski is not subcanonical}
\input{non-commutative.tex}

\section{Unramified surjective maps}
\input{etale.tex}

\section{Weakly closed sets}
\input{wclosed.tex}

\printindex

\printbibliography

\end{document}
