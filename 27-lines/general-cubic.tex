...
Then the type of lines in $C_z$ is projective. Let $\Gr(2,4)$ denote the type of rank \(2\) submodules of \(R^4\), which is equivalent to the type of lines in $\bP^3$ and
let \[Z\colonequiv\sum_{z:\bP^{19}}\sum_{l:\Gr(2,4)}l\subseteq C_z\rlap{.} \]
...
\begin{lemma}
  \label{smooth-cubics-open}
  The subtype $S\colonequiv \{ z:\bP^{19} \;|\; C_z \;\text{is smooth} \}$ is open.
\end{lemma}
...
\begin{proof}
For two lines \(l,l^\prime:\Gr(2,4)\) there merely is \(T:\PGL_{4}(R)\) such that \(T(l)=l^\prime\)\rednote{TODO: prove}.
...
\begin{lemma}
  \label{beta-unramified}
  \(\beta:Z\to S\) is unramified.
\end{lemma}
...
\begin{lemma}
  \label{smooth-dim-19}
  \(Z\) is smooth of dimension \(19\).
\end{lemma}

\begin{proof}
  By \Cref{p15-bundle}, the fibers of \(\alpha:Z\to \Gr(2,4)\) are merely \(\bP^{^15}\) and therefore smooth of dimension \(15\). \(\Gr(2,4)\) itsself is smooth of dimension \(4\).
  So \(Z\), the total space of \(\alpha\), must be smooth of dimension \(19\).
\end{proof}
...
\begin{proof}
  We show now that \(\beta\) is smooth.
  By \cite{diffgeo-article}[Lemma 2.2.7] the kernel of \(d\beta_x:T_xZ\to T_{\beta(x)}S\)
  is given by the tangent space \(T_{(x,\refl)}\fib_\beta(\beta(x))\).
  By \Cref{beta-unramified}, we know that this fiber is an unramified scheme, so all its tangent spaces  are \(\{0\}\).
  By \Cref{smooth-dim-19}, \(T_xZ\) is free of rank 19 and \(T_{\beta(x)}S\) is also free of rank 19,
  since \(S\) is an open subset of \(\bP^{19}\) by \Cref{smooth-cubics-open}, so the inclusion is étale and induces an isomorphism on tangent spaces.

  So \(\beta\) is unramified and smooth and therefore étale.
\end{proof}
