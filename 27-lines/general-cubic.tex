...
Then the typ of lines in $C_z$ is projective. Let $\mathrm{Lines}(3)$ denote the type of lines in $\bP^3$ and
let \[Z\colonequiv\sum_{z:\bP^{19}}\sum_{l:\mathrm{Lines}(3)}l\subseteq C_z\rlap{.} \]
be the type of lines on cubics.
We have a span:
\begin{center}
\begin{tikzcd}
...
\item \(Z\) is smooth, proper and irreducible.
...
\item \(Z\) is proper, since dependent sums of proper types are proper. Same for smooth and irreducible.
\item Since \(Z\) is proper, the proposition \((x\notin \im(\beta))=((z:Z)\to \beta(z)\neq x)\) is open. \(\bP^{19}\) is irreducible,
