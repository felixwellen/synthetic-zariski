...
\begin{definition}
  ...
  \[\delta_1(\beta)_x(y_1,y_2,y_3) = \beta_x(y_2,y_3)\cdot\beta_x(y_1,y_3)^{-1}\cdot\beta_x(y_1,y_2)\]
  We have that $\delta_1(\delta_0(\alpha)))_x(y_1,y_2,y_3) = 1$.
\end{definition}

First we give a couple of auxiliary results on \v{C}ech short sequence, justifying their usefulness.
...
by having $\beta_x(y_1,y_2) = \psi_x(y_1)^{-1} \cdot \psi_x(y_2)$. It is easy to check that it is a cocycle in the given sequence (meaning $\delta_1(\beta) = 1$), so that by exactness there exists some $\alpha:\prod_{x:X}G(x)^{Y(x)}$ such that for all $x:X$ and $y_1,y_2:Y(x)$ we have that:
...
following exact sequence:
  ...
  \pi_{k-1}(F) \to \cdots ,
  $$
  Form this plus the map $\pi_n(K(B,n))\to \pi_n(K(C,n))$ being surjective, we conclude that $F$ is $n-1$-connected and that $\pi_n(F) = A$. By unicity of deloopings we get $F=K(A,n)$.
