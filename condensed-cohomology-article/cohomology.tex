...
\begin{lemma}\label{LemmaExactnessAbove2}
...
\end{lemma}
\begin{proof}
  In this proof we make use of results from chapter VI of \cite{EilenbergSteenrod} and the references are to that chapter of that book. 
  ...
  So to show exactness of the above sequence, it's sufficient to show exactness of the sequence $Hom(D_k,G)$, where 
$D_k$ is the quotient of the free group on $\mathbb I_n^{\sim k}$ by the degenerate simplices. 
...
\end{proof}
\begin{lemma}
  If $G$ is abelian and overtly discrete, the sequence in \Cref{EqSequence} is exact. 
\rednote{I'm not sure I actually need abelian and overtly discrete?. We needed overtly discrete to make the cohomology agree with the cech cohomology, but the argument below doesn't use commutativity I think}
\end{lemma}
\begin{proof}
...
%
 ...
 $$\beta(a) = \alpha(a-1,a) +  \alpha (a-2,a-1) + \cdots + \alpha(0,1).$$
  ...
  Thus $d_0(\beta) $ and $ \beta$ on all possible inputs of $\mathbb I_n^{\sim 1}$
  and we have exactness at $G^{\mathbb I_n^{\sim1}}$.
%
  By \Cref{LemmaExactnessAbove2}, we have exactness at all other objects in the sequence. 
\end{proof}
