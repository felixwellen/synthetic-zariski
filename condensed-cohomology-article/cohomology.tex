...
\begin{itemize}
\item A coboundary is a $\delta:\Pi_{x:X}G(x)^{Y(x)^2}$ such that for all $x:X$ and $y_0,y_1,y_2:Y(x)$ we have $\delta_x(y_0,y_1)\cdot \delta_x(y_1,y_2) = \delta_x(y_0,y_2)$.
\item Two coboundaries $\delta$ and $\delta'$ are called cohomologous if there exists $\gamma:\Pi_{x:X}G(x)^{Y(x)}$ such that for all $x:X$ and $y_0,y_1:Y(x)$ we have $\gamma_x(y_0)\cdot\delta_x(y_0,y_1) = \delta'_x(y_0,y_1)\cdot\gamma_x(y_1)$.
\end{itemize}
\end{definition}
...
\begin{definition}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. The \v{C}ech non-abelian cohomology of $X$ with coefficient in $G$ is defined as coboundaries modulo being cohomologous. It is denoted by $\check{H}^1$(X,Y,G).
\end{definition}

We prove a first easy lemma.

\begin{lemma}\label{first-cech-has-section-vanish}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. If we merely have $\Pi_{x:X}Y(x)$ then $\check{H}^1(X,Y,G)$.
\end{lemma}
...
Our main goal is to show that \v{C}ech cohomology corresponds to $G$-torsors over $X$ that are trivialised by $Y$. More precisely:

\begin{definition}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. We define $H^1(X,Y,G)$ as $\{\alpha:H^1(X,G)\ |\ \pi^*(\alpha) = 0 \}$ where $\pi$ is the projection $\Sigma_{x:X}Y(x)\to X$.
\end{definition}

Our goal is to show $H^(X,Y,G) = \check{H}^1(X,Y,G)$.

\begin{lemma}\label{phi-well-defined}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. Given $\alpha:\Pi_{x:X}BG(x)$ and $\beta:\Pi_{x:X}\alpha(x)^{Y(x)}$, we define $\phi(\alpha,\beta) = \lambda x,y_0,y_1.\, \beta_x(y_0)^{-1}\cdot\beta_x(y_1)$. Then $\phi(\alpha,\beta)$ is a coboundary and moreover given another $\beta':\Pi_{x:X}\alpha(x)^{Y(x)}$, we have that $\phi(\alpha,\beta)$ and $\phi(\alpha,\beta')$ are cohomologous.
\end{lemma}

\begin{proof}
It is clear that $\phi(\alpha,\beta)$ is a coboundary. To show that $\phi(\alpha,\beta)$ and $\phi(\alpha,\beta')$ are cohomologous, we consider $\gamma = \lambda x,y.\, \beta'_x(y)^{-1}\cdot\beta_x(y)$. Given $x:X$ and $y_0,y_1:Y(x)$ we indeed have:
...
\begin{definition}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. The map induced by \Cref{phi-well-defined} is denoted by $\Phi : H^1(X,Y,G)\to \check{H}^1(X,Y,G)$.
\end{definition}

\begin{lemma}\label{psi-well-defined}
Assume given $Y$ an inhabited type and $G$ a group, as well as $\delta:G^{Y^2}$ a coboundary. %such that for all $y_0,y_1,y_2:Y$ we have $\delta(y_0,y_1)\cdot\delta(y_1,y_2) = \delta(y_0,y_2)$. 
Then we define $\psi(\delta) = \{\gamma:G^Y\ |\ \forall y_0,y_1.\, \gamma(y_0)\cdot\gamma(y_1)^{-1} = \delta(y_0,y_1)\}$. Then:
\begin{enumerate}[(i)] 
\item For all $x:X$, the type $\psi(\delta)$ is indeed a $G$-torsor.
\item We have a $H:\psi(\delta)^Y$ defined by $H_x(\bar{y}) = \lambda y.\delta(y,\bar{y})$.
\item Given $\delta$ and $\delta'$ with $\iota:G^Y$ such that for all $y_0,y_1:Y$ we have $\iota(y_0)\cdot\delta(y_0,y_1) = \delta(y_0,y_1)\cdot\iota(y_1)$, we have an equivalence $\psi(\delta)=\psi(\delta')$.
\end{enumerate}
\end{lemma}
...
The fact that $\propTrunc{\psi(\delta)}$ follows directly from $Y$ inhabited and (ii).

\item We just need to check that for all $\bar{y},y_0,y_1$ we indeed have $\delta(y_0,\bar{y})\cdot\delta(y_1,\bar{y})^{-1} = \delta(y_0,y_1)$, but this follows from $\delta$ being a coboundary.

\item The map $G:\psi(\delta)\to \psi(\delta')$ sending $\gamma$ to $\lambda y. \iota(y)\cdot\gamma(y)$ is clearly an equivalence.
\end{itemize}
\end{proof}

\begin{definition}
Assume given $X$ a type, $Y(x)$ an inhabited type and $G(x)$ a group both depending on $x:X$. The map induced by \Cref{psi-well-defined} is denoted by $\Psi : \check{H}^1(X,Y,G)\to H^1(X,Y,G)$.
\end{definition}

Now we just want to check that $\Phi$ and $\Psi$ are inverse to each other.

\begin{lemma}\label{psi-phi-is-id}
We have that $\Psi\circ\Phi = Id$.
\end{lemma}

\begin{proof}
It is enough to assume $\alpha:\Pi_{x:X}BG(x)$ with $\beta:\Pi_{x:X}\alpha(x)^{Y(x)}$ and prove that for all $x:X$ we have 
...
\begin{proof}
By Lemmas \ref{psi-phi-is-id} and \ref{phi-psi-is-id}.
\end{proof}
...
Assume given $S$ a Stone space and $(T_k(x))_{k:\N}$ a tower of Stone spaces and $G(x)$ an overly discrete group both depending on $x:S$. Write $T(x) =\mathrm{lim}_kT_k(x)$. Then $\check(H)^1(S,T,G) = \mathrm{colim}_k \check{H}^1(S,T_k,G)$.
\end{lemma}
...
\begin{remark}
A set $X$ merely has a \v{C}ech cover if and only is it is a compact Hausdorff space. \rednote{TODO proof}
\end{remark}

\begin{proposition}
Assume given $X$ a compact Hausdorff space with a \v{C}ech cover $S$ and $G(x)$ an overtly discrete group depending on $x:X$. Then $H^1(X,G) = \check{H}(X,S,G)$.
\end{proposition}
