...
\begin{remark}
  Let $G$ be a group. 
  We write $BG$ for the delooping of $G$ 
  \cite{hott,davidw23}. 
  Recall that $BG$ is a type with a term 
  $* : BG$ such that group $* = *$ is equal to $G$. 
  Recall also that $H^1(X,G)$ 
  is the set truncation of $X \to \B G$. 
\end{remark} 
...
\begin{proof}
  ...
  we can write $\pi$ as limit of surjections between finite sets, 
  which we write $\pi_k : Q_k\twoheadrightarrow S_k$. 
  ...
  giving the required section $\Pi_{x :S} T_k(x)$. 
\end{proof}
