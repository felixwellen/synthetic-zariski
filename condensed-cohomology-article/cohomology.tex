...
\end{remark}
\begin{definition}
  We recall from Definition VI.2.3 of \cite{EilenbergSteenrod} that there is a ordered chain complex $C_k$ associated to $\mathbb I_n^{\sim}$. For each $k$, the group $C_k$ is given by the free abelian group on $\mathbb I_n^{\sim k}$, and the map $\partial_k: C_k\to C_{k-1}$ is induced by 
  $$\partial_k(a_0,\cdots , a_k) = \sum\limits_{i=0}^k (-1)^i(a_0,\cdots \widehat{a_i},\cdots a_k).$$

  We also recall from Definition VI.6.8 of \cite{EilenbergSteenrod} that there is an alternating chain complex $D_k$ associated to $\mathbb I_n^\sim$, which for each $k$ has the group $D_k$ given by quotienting out degenerate elements from $C_k$. 
\end{definition}
\begin{remark}\label{RmkTrivialDk}
  As $\mathbb I_n^{\sim k}$ contains at most 2 sequences, by Lemma VI.6.6 of \cite{EilenbergSteenrod}, for $k\geq 2$ all simplices of $C_k$ are degenerate, and hence $D_k$ is trivial. 
\end{remark}
...
\begin{proof}
  ...
  By Theorem VI.6.10 of \cite{EilenbergSteenrod}, this ordered chain complex is homotopy equivalent to the alternating chain complex $D_k$. As $D_k$ was trivial for $k\geq 2$ by \Cref{RmkTrivialDk}, $Hom_{Ab}(D_k,G)$ is also trivial.
