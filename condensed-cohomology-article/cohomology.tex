...
\end{remark}
\begin{definition}
  Define the ordered chain complex \parencite[Def VI.2.3]{EilenbergSteenrod} $C_k$ associated to $\mathbb I_n^{\sim}$. For each $k$, the group $C_k$ is given by the free abelian group on $\mathbb I_n^{\sim k}$, and the map $\partial_k: C_k\to C_{k-1}$ is induced by 
  $$\partial_k(a_0,\cdots , a_k) = \sum\limits_{i=0}^k (-1)^i(a_0,\cdots \widehat{a_i},\cdots a_k).$$

  Define also the alternating chain complex \parencite[Def VI.6.8]{EilenbergSteenrod} $D_k$ associated to $\mathbb I_n^\sim$, which for each $k$ has the group $D_k$ given by quotienting out degenerate elements from $C_k$. 
\end{definition}
\begin{remark}\label{RmkTrivialDk}
As $\mathbb I_n^{\sim k}$ contains at most 2 sequences, by \parencite[Lemma VI.6.6]{EilenbergSteenrod}, for $k\geq 2$ all simplices of $C_k$ are degenerate, and hence $D_k$ is trivial. 
\end{remark}
