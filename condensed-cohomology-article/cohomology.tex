...
\begin{definition}
  ...
  We denote $\mathbb I_n^{\sim k}$ for the set of elementary $k$-chains of $\mathbb I_n^\sim$ as in Definition VI.2.1 of \cite{EilenbergSteenrod}.
\end{definition}   
...
\begin{remark}\label{description-Cn-simn}
  ...
  There are at most two such sequences, which means the simplicial complex $I_n^{\sim}$ is 1-dimensional in the sense of Definition II.3.2 of \cite{EilenbergSteenrod}. 
...
\end{lemma} 
\begin{proof}
 We make use of results from chapter VI of \cite{EilenbergSteenrod} and all the references in this proof are to that chapter of that book. We note that the reasoning in that chapter is fully constructive; it makes use of an order with trichotomy, but this order is assigned to a finite set. 
The sequence we are considering is given by applying $Hom_{Ab}(-,G)$ applied to the ordered chain complex (Definition 2.3) corresponding to $\mathbb I_n^\sim$, which had as $k$'th object the free abelian\footnote{Note that in the reference definition 2.1, the word "abelian" is omitted, but otherwise the proof in 2.2 wouldn't make sense.} group on generators from $\mathbb I_n^{\sim k}$, which we denote $C_k$. 
  By Theorem 6.10, this ordered chain complex is homotopy equivalent to the alternating chain complex where the objects $D_k$ are given by quotienting out the degenerate simplices from $C_k$. 
  Now as $I_n^\sim$ is 1-dimensional, by Lemma 6.6, any $k$-simplex of $C_k$ with $k\geq 2$ is degenerate, hence $D_k$ is trivial, and so is $Hom_{Ab}(D_k,G)$. In particular, for $k\geq 2$, the sequence is exact at $Hom_{Ab}(D_k,G)$, and thus the required sequence is exact at $G^{\mathbb I_n^{\sim k}}$.
\end{proof}
