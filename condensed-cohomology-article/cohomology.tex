...
\begin{lemma}\label{ExactSequenceDelooping}
  Let $A \to B \to C\to 0$ be an exact sequence of abelian groups. 
...
\begin{proof}
  ...
  \[ \cdots \to 
    ...
    we have $K(A_x^{S_x},n) = \prod_{t:S_x}K(A_x,n)$. 
    But now note that as $\Sigma_{x:X} S_x = S$, we have
    $\prod_{x:X}\prod_{t:S_x}K(A_x,n) = 
     \prod_{s : S} K(A_{fs},n)$, 
     and as $n\geq 1$, by \Cref{vanishing-cohomology-stone},
     the set truncatation of the latter is indeed $0$, 
     as required. 
\end{proof}
