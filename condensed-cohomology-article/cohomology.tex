...
\rednote{Probably better phrased as simply for all A, S we have that $K(A^S,n) \simeq K(A,n)^S$ inductively, with cohomology as corollary.}
...
where $\alpha$ is send to $0$ in $H^n(x:S,A_x^{T_x})$. By induction we have $H^{n-1}(x:S,L_x)=0$ from which we conclude that $\alpha$ is equal to $0$ in $H^n(x:S,A_x)$.
\end{proof}
