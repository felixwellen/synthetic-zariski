...
\begin{lemma}%\label{LemmaIntervalExactnessforkabove2}
  Fix a group $G$ and natural number $n$. 
  Consider the sequence
  \begin{equation}\label{EqSequence}
    \begin{tikzcd}
      0 \to G \to G^{\mathbb I_n^{\sim 0}  } \arrow [r , "d_0"]     & 
...
\begin{proof}
  ...
  If $\alpha : \mathbb I_n \to G$ is such that 
  $\alpha(b) -\alpha(a) = 0$ whenever $ a \sim_n b$, 
  then $\alpha$ respects  $\sim_n$. 
  By \rednote{TODO, is now presented later}, any map $\mathbb I_n \to G$ respecting $\sim_n$ is constant. Thus we have exactness at $G^{\mathbb I_n^{\sim 0}}$. 

  If $\alpha : \mathbb I_n^{\sim 1} \to G$ is such that 
  $\alpha(b,c) -\alpha(a,c) + \alpha (a,b) = 0$, then define 
  $\beta : \mathbb I_n \to G$ by 
  $\beta(a) = \alpha (a,a-1) + \alpha (a-1,a-2) + \cdots + \alpha (1,0)$. 
  Then, for $b = a + 1$, we have 
  $\beta(b) -\beta(a) = \alpha(b,a)$. 
  For $b = a$, we have $\beta(b) -\beta(a) = 0 =?? \beta(b,a)$ \rednote{TODO I think this argument works in the alternating case in general, not requiring abelian stuff. check after sleep}
  For $b = a -1$, we have $\beta(b) - \beta (a) = - \alpha (a,b)$
\end{proof}
