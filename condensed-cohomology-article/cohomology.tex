...
\begin{proof}
  ...
  Thus $\alpha$ is in the kernel of $\prod_{x:S}K(A_x,k) \to K(A_x^{T_x},k)$.
  By exactness $\alpha$ is in the image $\prod_{x:S} K(L_x,k-1) \to \prod_{x:S}K(A_x,k)$. 
  As $n\geq 1$, we can use the induction hypothesis and \Cref{ev-eq-acyclic} to see that $H^n(S,L)=0$.
  Thus any $\beta:\prod_{x:S}K(L_x,k-1)$ equals $*$, 
  so their image equals $*$ in $\prod_{x:S}K(A_x,k)$. 
