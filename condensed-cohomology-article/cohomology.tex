...
\begin{remark}\label{description-Cn-simn}
  ...
  There are at most two such sequences, so $I_n^{\sim}$ is 2-dimensional. 
  Also note that for $cs : 2^\mathbb N \twoheadrightarrow \mathbb I$, we have 
  $$\Sigma_{x : \mathbb I} (cs^{-1}(x))^{k+1} = \lim_{n} \mathbb I_n^{\sim k}$$
\end{remark}
%\begin{remark}\label{RmkConstantIfRespsimn}
%  Recall that the transitive closure of $\sim_n$ relates all the elements of $2^n$. Therefore, any map out of $2^n$ respecting $\sim_n$ is constant. 
%\end{remark}

\begin{lemma}\label{LemmaExactnessIntervalApprox}
  Fix a group $G$ and natural number $n$. 
  ...
  \begin{equation}\label{EqSequence}
    ...
    \end{tikzcd}
  \end{equation}
  with $d_k(f)(a_0, \cdots , a_k) = \sum\limits_{i=0}^k (-1)^i f(a_0 , \cdots, \widehat {a_i} , \cdots, a_k)$. 
  This sequence is exact. %at $G^{\mathbb I_n^{\sim k}}$ for $k\geq 2$. 
\end{lemma}
\begin{proof}
...
%
\item  In the rest of this proof we make use of results from chapter VI of \cite{EilenbergSteenrod} and the references are to that chapter of that book. 
  Note that the above sequence is $Hom(-,G)$ applied to the ordered chain complex (Definition 2.3) corresponding to $\mathbb I_n^\sim$. 
...
$D_k$ a quotient of the free group on $\mathbb I_n^{\sim k}$ by the degenerate simplices. 
  Now as $I_n^\sim$ is 2-dimensional, by Lemma 6.6, any $k$-simplex with $k\geq 2$ is degenerate, hence $D_k$ is trivial, and so is $Hom(D_k,G)$. In particular, for $k\geq 2$, the sequence is exact at $Hom(D_k,G)$ as required. 
\end{itemize}
\end{proof}
...
Then we have that $\mathrm{lim}_n\I_n^{\sim l} = \Sigma_{x:\I} T(x)^l$. By \Cref{LemmaExactnessIntervalApprox} and stability of exactness under sequential colimit, we have an exact sequences:
...
\item Same as point (ii) using \Cref{cech-and-sheaf-agree} instead of \ref{non-abelian-cech-and-sheaf-agree}.
