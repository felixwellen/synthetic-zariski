...
\[ev_n^{S,A} : K(\prod_{x:S}A(x),n)\to \prod_{x:S}K(A(x),n)\]
is an equivalence.
\end{theorem}
...
\begin{proof}
  ...
  $ev_n^{S,A}$ is an equivalence and consider 
  $ev_{n+1}^{S,A}$ for some $S,A$.
  We will first show it is an embedding for all $S,A$. 
  and use this to show it is also surjective for all $S,A$. 

  To show $ev_{n+1}^{S,A}$ is an embedding, 
...
\[ev_n^{S,A} : K(\prod_{x:S}A(x),n) \to \prod_{x:S}K(A(x),n)\]
so we conclude by induction hypothesis that $ev_{n+1}^{S,A}$ 
is an embedding for all Stone spaces $S$, 
and $S$-indexed Abelian groups $A$. 

To show surjectivity, note that 
$\prod_{x:S}K(A(x),n+1)$ is connected, 
so it's sufficient (\cite{hott, TODO}) to show that 
$H^{n+1}(S,A) = 0$ to conclude. 
...
For $x:X$, denote $L_x$ for $A_x^{T_x}/A_x$, that is the quotient of $A_x^{T_x}$ over the constant map $A_x \to A_x^{T_x}$. 

\rednote{The following should be standard, we need a reference}
Note that $A_x^{T_x} \to L_x$ induces a map 
$K(A_x^{T_x},n) \to K(A_x^{T_x},n)$. 
Following definition 8.4.3 of \cite{hott}, we get the 
following long exact sequence in $k$:
  $$
  \cdots \to \pi_{k+1}(K(L_x,n)) \to 
  \pi_{k}(F) \to \pi_{k}(K(A_x^{T_x},n)) \to \pi_{k}(K(L_x,n)) \to 
  \pi_{k-1}F \to \cdots ,
  $$
where $F$ is the fiber of $K(A_x^{T_x},n) \to K(L_x,n)$. 

Taking $k =n-1$, we have that 
$$\pi_{n}K(A_x^{T_x},n) \to \pi_nK(L_x,n) \to \pi_{n-1} (F) \to \pi_{n-1}K(A_x^{T_x},n)$$
is exact, hence 
$\pi_n(F) \to A_x^{T_x} \to L_x \to \pi_{n-1}(F) \to 0$ is exact. 
As $A_x^{T_x} \to L_x$ is surjective, 
we have that $\pi_{n-1}(F) = 0$. 
Thus $F$ is $n-1$-connected. 
Also $F$ is an $n$-type as it's 
the fiber of a map of $n$-types.
Thus $\pi_n(F)$ is a set and equals $A$. 
Therefore $F = K(A,n)$. 
We conclude that we have an exact sequence of the form
$$\cdots\to K(L_x,k+1) \to 
  K(A_x^{T_x},k) \to K(A_x, k) \to K(L_x,k) \to
  K(A_x^{T_x},k-1) \to \cdots
$$
Now as exact sequences commute with $\Pi$,
we also get an exact sequence
$$\cdots\to 
\Pi_{x:S}K(L_x,k+1) \to 
\Pi_{x:S}K(A_x^{T_x}) \to \Pi_{x:S}K(A_x, k) \to \Pi_{x:S}K(L_x,k) \to
 \Pi_{x:S} K(A_x^{T_x}) \to \cdots
$$
And as exact sequences commute with set truncation 
(Theorem 8.4.6 of \cite{hott}), we also get the exact sequence
$$\cdots\to H^{n+1}(S,L) \to 
H^{n}(S,\lambda x . A_x^{T_x})\to H^n(S,A) \to H^n(S,L)\to
H^{n}(S,\lambda x . A_x^{T_x}) \to \cdots
$$

By the induction hypothesis, we have 
$\Pi_{x : S} K(A_x^{T_x},n) = K(\Pi_{x:S}A_x,n)^{T_x}$, 
hence for $n>0$, we have 
$||\Pi_{x : S} K(A_x^{T_x},n)||_0 = 0$, 
...
$\Pi_{x:S}K(A_x,n) \to \pi_{x:S} K(A_x,n)^{T_x}$:
...
$ev_n^{T_x,\lambda t . A_x}$), 
which is an embedding by the above. 
As any $\alpha : \Pi_{x :S} K(A_x,n)$ becomes $0$ in $\Pi_{x : S} K(A_x,n)^{T_x}$, by the last map being an embedding, it also becomes $0$ in $\Pi_{x : S} K(A_x^{T_x},n)$.
