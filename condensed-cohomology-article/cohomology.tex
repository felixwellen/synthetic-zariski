...
\begin{lemma}\label{ExactSequenceDelooping}
  Let $A \to B \to C\to 0$ be a fiber sequence of abelian groups. 
...
\begin{proof}
  ...
  Thus any $\beta:\prod_{x:S}K(L_x,n)$ equals $*$, 
  so their image equals $*$ in $\prod_{x:S}K(A_x,n+1)$. 
  Thus $\alpha = *$, as required. 
\end{proof}
...
\[\delta^n(\alpha)(x,u_0,\cdots,u_n) = \sum_{i=0}^n (-1)^i\alpha(x,u_0,\cdots,\hat{u_i},\cdots,u_n)\]
Then the \v{C}ech cohomology:
\[\check{H}^k(x:X,A_x)\]
is defined as the $k$-th homology group of the \v{C}ech complex, 
so as the image of $\delta^n$ by the kernel of $\delta^{n+1}$. 
\end{definition}
