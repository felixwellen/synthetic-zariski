...
\begin{definition}
  ...
  We denote $\mathbb I_n^{\sim k}$ for the elementary $k$-chains of $\mathbb I_n^\sim$ as in \cite{EilenbergSteenrod, Definition VI.2.1}.
\end{definition}   
\begin{remark}
  Any face of $\mathbb I_n^\sim$ contains at most 2 vertices. Therefore, any elementary $k$-chain of $\mathbb I_n^{\sim}$ is degenerate for $k >2$. 
\end{remark}
\begin{lemma}
  For any group $G$, we have that 
  $G^{\mathbb I_n^{\sim k}}$ TODO write exactness at $k > 2$. 
\end{lemma}
\begin{lemma}
\end{lemma}
...
We define $\I_n^{\sim k} = \{x_1,\hdots,x_k:\I_n\ |\ \forall(i,j).\, x_i\sim_n x_j\}$, so $k$-tuples in $2^n$ of terms that are related by $\sim_n$. 
Note that 
$$\Sigma_{x : \mathbb I} (cs^{-1}(x))^k = \lim_{n} \mathbb I_n^{\sim k}$$
\rednote{Now doubled, I think it makes sense to present it earlier to motivate $\mathbb I_n^{sim k}$.}

\begin{lemma}\Cref{LemmaInverseCsAtMostSize2}
  Let $n:\mathbb N$, and $a,b,c : 2^n$ be such that $a\sim_n b , b \sim_n c$, and $a\sim_n c$. 
  Then $a = b \vee b = c \vee a = c$. 
\end{lemma}
\begin{proof}
  Suppose for $x,y,z: Fin(2^n)$, that $|x-y| \leq 1,|y-z| \leq 1 , |x-z|\leq 1$.  
  We cannot have all of $x\neq y, x \neq z , y \neq z$.
  As equality is decidable in $Fin(2^n)$, we get that $x = y \vee x = z \vee y = z$. 
  Under the equivalence in \Cref{description-Cn-simn}, we conclude. 
\end{proof}
...
\rednote{ This theorem statement is not very nice, I wanted something of the form every cocycle is cohomologous in some cover (as in \Cref{dfnCocycle}), but that's not exactly the thing. We could reformulate the start of 3.1 or present $\I_n^{\sim k}$ with some first element fixed. Now I just change $\I_n^{\sim_k}$ to also involve an $x$ somewhere, but that shouldn't be called $\I_n^{\sim k}$, the $k$ has nothing to do with the $\sim$, but with the dimension of the tuple, and the $x$ is important. also lot's of other cleanup TODO}
  For $x:\I$, define $\mathbb I_{x, n}^{\sim k}$ to be those terms $\alpha : \mathbb I_n^{\sim k}$ such that 
  $|cs_n(\alpha) - x | \leq \frac1{2^n}$. Alternatively, these are $\alpha: 2^n$ that have an extension to an infinite sequence that converges to $X$. These are defined so that $(cs^{-1}(x))^k = lim \mathbb I_{x,n}^{\sim k}$. 

  Let $G$ be a group. 
  Let $\delta : G^{\I_{x,n}^{\sim 2}}$ be 
  such that $\delta(a,b) \cdot \delta(b,c) = \delta(a,c)$
  for all $a,b,c : \I_n$.
  Then there is some $\gamma : G^{I_{x,n}^{\sim 1}}$ such that 
  $\gamma(a) \cdot  \delta(a,b) = \gamma(b)$ 
  for all $x,y:\I_n$.
%Consider 
%\[G^{\I_n} \overset{\delta_0}{\longrightarrow} G^{\I_n^{\sim 2}} \overset{\delta_1}{\longrightarrow} G^{\I_n^{\sim 3}} \]
%  where:
%  \begin{eqnarray}
...
\begin{proof}
  Since $x\in \mathbb I$ and $cs:2^\mathbb N \to \I$ is surjective, 
  there exists some $\alpha :2^\mathbb N$ with $cs(\alpha) = x$. 
  Let $t = \alpha |_n$. 
  Take $\gamma(a) = \delta(t,a)$. Then as required, we have
  $$
   \gamma(a)   \cdot \delta(a,b) = 
   \delta(t,a) \cdot \delta(a,b) = 
   \delta(t,b) = \gamma(b).
  $$
\end{proof}
...
%  \rednote{
%  In this remark, $\mathbb I_{x,n} = \{a : 2^n | (x-cs_n(\alpha)) \leq \frac1{2^n}\}$. 
%    I don't know how we can transform this proof to use $\mathbb I_n^{\sim k}$ instead of $(\mathbb I_{x,n})^k$.  
%  The proof of \cite{EilenbergSteenrod} works when the chain complex is of the form $G^{F^k}$ where $F$ is some set on which we can place a trichotomy (a strict order st $x < y \vee x = y \vee x > y)$, which I've now called a strict linear order. Note that $\mathbb I_n^{\sim k}$ is not of such a form. $\mathbb I_{x,n}$ works for $F$, but note it is not a finite set, as we cannot decide whether it's size 1 or 2. 
%}
%
%  \begin{lemma}
%    For each group $G$, $x:\mathbb I, n,k : \mathbb N$ with $k\geq 2$, the sequence 
%    \begin{equation*}
%      \begin{tikzcd}
%        G^{\mathbb I_{x,n}^{k}} \arrow[r,"d_{k}"] &
%        G^{\mathbb I_{x,n}^{k+1}} \arrow[r,"d_{k+1}"] &
%        G^{\mathbb I_{x,n}^{k+2}}  
%      \end{tikzcd}
%    \end{equation*}
%    with $d_{k}(f)(a_0,\cdots , a_k) = \sum\limits_{i=0}^{k+1} 
%    f(a_0, \cdots , \widehat{a_i}, \cdots , a_k)$, is exact. 
%  \end{lemma}
%
%  \begin{remark}
%%  Recall the notions of chain complex, and alternating chain complex, associated to a finite set (Definitions VI.2.1 and VI.6.8 of \cite{EilenbergSteenrod}).
%%  The chain complex is the sequence $\cdots \to C_{k+1}\to C_k \to C_{k-1} \to \cdots$ where $C_{k}$ is the free group with generators tuples from $(\mathbb I_{x,n})^{k+1}$ and the maps $d_k: C_{k+1}\to C_k$ is induced by sending $(c_0, \cdots , c_k) \mapsto \sum\limits_{0\leq i \leq k} (c_0, \cdots, \widehat{c_i} , \cdots , c_k)$. 
%%  The alternating chain complex is the sequence $D_{k+1}\to D_k \to D_{k-1}$ where 
%%  $D_k$ is the quotient of $C_k$ by it's degenerate elements. 
%%  The definition of degenerate elements (VI.6.1) I won't repeat for now, but important is that if two entries in a generator $(c_0, \cdots c_k)$ are repeated, then that generator is degenerate (VI.6.6). 
%    Note that for all $x:\mathbb I$, the type $\mathbb I_{x,n}$ has a strict linear order. 
%    Therefore the proof of \cite{EilenbergSteenrod, Theorem VI.6.10} goes trough for the chain complex associated to $\mathbb I_{x,n}$. 
%    So for $C_k$ the free group on $(\mathbb I_{x,n})^{k+1}$, 
%    turning into a chain complex $C$ under the canonical boundary maps \cite{EilenbergSteenrod, Definition VI.2.1}, there is a homotopy equivalence between $C$ and the same sequence quotiented out by degenerate chains \cite{EilenbergSteenrod, Definition VI.6.8}.
%    As $\mathbb I_{x,n}$ has at most two elements, any element of $C_k$ is degenerate for $k\geq 2$ \cite{EilenbergSteenrod, Lemma VI.6.6}.
%
%
%    Transporting this equivalence 
%
%  Thus there is a homotopy equivalence between the chain complex and the alternating chain complex. Transporting this equivalence along the functor $Hom(-,G) : Grp \to Grp$ gives us a homotopy equivalence between the sequences $[G,{D_{k+1}}] \leftarrow [G,{D_k}] \leftarrow [G,{D_{k-1}}]$ and $[G,{C_{k+1}}] \leftarrow [G,{C_k}] \leftarrow [G,{C_{k-1}}]$. 
%
%  
%
%
...
\begin{proof}
  Exactness at $k=1$ is above. For $k\geq2$, note that by \Cref{LemmaInverseCsAtMostSize2}, for any tuple $t : \I_{x,n}^{\sim k}$, there are at least 2 entries of $t$ which are equal. 




  Now we refer to Stacks Tag 01FG or Serre DAC 20, proposition 2. These give proofs that the cohomology we defined in \Cref{dfnCech} agrees with an alternating \v Cech cohomology. 
  \rednote{Add Eilenberg Steenrod 6.10}
  As $I_{x,n}^{\sim k}$ is a finite set, we can assign to it an order with trichotomy, and the proofs in these references go trough. \rednote{TODO this we should explain in more detail}
  Now an alternating \v Cech cohomology must be 0 if two of the same entries occur. Therefore, the sequence is exact. 
