...
\begin{proof}
Assume given $\delta:\Pi_{x:X}G(x)^{Y(x)^2}$ a coboundary, we need to show it is cohomologous to $1$. It is enough to consider $\gamma:\Pi_{x:X}G(x)^{Y(x)}$ defined by $\gamma_x(y) = \delta_x(t_x,y)$.
\end{proof}
...
\begin{enumerate}[(i)]
\item $\phi(\alpha,\beta)$ is a coboundary.
...
\item Given $\delta$ and $\delta'$ coboundaries with $\iota:G^Y$ such that for all $y_0,y_1:Y$ we have $\iota(y_0)\cdot\delta(y_0,y_1) = \delta(y_0,y_1)\cdot\iota(y_1)$, we have $\psi(\delta)=\psi(\delta')$.
