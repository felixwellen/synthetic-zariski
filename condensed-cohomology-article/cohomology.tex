...
\begin{proof}
  Exactness at $k=1$ is above. For $k\geq2$, note that by \Cref{LemmaInverseCsAtMostSize2}, for any tuple $t : \I_{x,n}^{\sim k}$, there are at least 2 entries of $t$ which are equal. Now we refer to Stacks Tag 01FG or Serre DAC 20, proposition 2. These give proofs that the cohomology we defined agrees with an alternating \v Cech cohomology. As $I_{x,n}^{\sim k}$ is a finite set, we can assign to it an order with trichotomy, and the proofs in these references go trough. Now an alternating \v Cech cohomology must be 0 if two of the same entries occur. Therefore, the sequence is exact. 
