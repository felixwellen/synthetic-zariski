...
Assume given a type $X$ with $Y(x)$ a type and $G(x)$ a group depending on $x:X$. If we merely have $\prod_{x:X}Y(x)$, then $\check{C}(X,Y,G)$ is exact.
\end{lemma}
...
\begin{proof}
 Identifying $* =_{BG(x)} *$ and $G(x)$, we define $\beta : \prod_{x:X} G(x)^{T(x)^2}$ by:
 \[\beta_x(y_1,y_2) = \psi_x(y_0)^{-1} \cdot \psi_x(y_1)\] 
 It is easy to check that it is a cocycle in the given sequence (meaning $\delta_1(\beta) = 1$), so that by exactness it is a coboundary, i.e. there exists some $\alpha:\prod_{x:X}G(x)^{Y(x)}$ such that for all $x:X$ and $y_0,y_1:Y(x)$ we have that:
\[\beta_x(y_0,y_1)=\alpha_x(y_0)^{-1}\cdot \alpha_x(y_1)\]
Then we define $\psi' : \prod_{x:X}(\phi(x)=*)^{Y(x)}$ by:
...
\begin{proof}
  ...
  Thus $\alpha$ is in the kernel of $\prod_{x:S}K(A_x,n+1) \to K(A_x^{T_x},n+1)$.
  By exactness $\alpha$ is in the image $\prod_{x:S} K(L_x,n) \to \prod_{x:S}K(A_x,n+1)$. 
  As $n\geq 1$, we can use the induction hypothesis and \Cref{ev-eq-acyclic} to see that $H^n(S,L)=0$.
  Thus any $\beta:\prod_{x:S}K(L_x,n)$ merely equals $*$, 
  so their image merely equals $*$ in $\prod_{x:S}K(A_x,n+1)$. 
...
\begin{proof}
  ...
  \[ \cdots \to 
    ...
    we have $K(A_x^{S_x},n) = \prod_{t:S_x}K(A_x,n)$. Thus
    $$
      H^n(X,\lambda x .A_x^{S_x})= 
      ||\prod_{x:X}K(A_x^{S_x},n)||_0 = 
      ||\prod_{x:X}\prod_{t:S_x}K(A_x,n)||_0 = 
      ||\prod_{s : S} K(A_{fs},n)||_0 = 
...
\[\prod_{x:X}A(x)^{Y(x)} \overset{\delta_0}{\longrightarrow} \prod_{x:X}G(x)^{A(x)^2} \overset{\delta_1}{\longrightarrow} \prod_{x:X}A(x)^{Y(x)^3} \overset{\delta_3}{\longrightarrow} \cdots \]
with:
...
\begin{proof}
Indeed assume given:
\[\alpha : \prod_{x:X} Y(x)^{n+1}\to A(x)^{Y(x)}\]
...
Then we define:
\[\beta : \prod_{x:X} Y(x)^{n}\to A(x)^{Y(x)}\]
