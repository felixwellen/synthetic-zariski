...
%  Note that for each $n:\mathbb N, x : \mathbb I$, 
%  $\mathbb I_{x,n}$ is a finite set with at most two elements and has an order with tricohtomy.
  \rednote{I don't know how we can transform this proof to use $\mathbb I_n^{\sim k}$ instead of $(\mathbb I_{x,n})^k$. 
  The proof of \cite{EilenbergSteenrod} works when the chain complex is of the form $G^{F^k}$ where $F$ is some finite set. Note that $\mathbb I_n^{\sim k}$ is not of such a form.}
  ...
  As $\mathbb I_{x,n}$ is finite, the proof of Theorem VI,6.10 of \cite{EilenbergSteenrod} goes trough constructively and there is a homotopy equivalence between the chain complex and the alternating chain complex. Transporting this equivalence along the functor $Hom(-,G) : Grp \to Grp$ gives us a homotopy equivalence between the sequences $[G,{D_{k+1}}] \leftarrow [G,{D_k}] \leftarrow [G,{D_{k-1}}]$ and $[G,{C_{k+1}}] \leftarrow [G,{C_k}] \leftarrow [G,{C_{k-1}}]$. 


  As $\mathbb I_{x,n}$ has at most 2 elements, for $k\geq 2$, any element of $(\mathbb I_{x,n})^{k+1}$ is degenerate as generator in $C_k$. 
  Thus $D_k$ is the trivial group $0$, and so is $[G,{D_k}]$.
  Note by the way that by the universal properties of freely generated groups, we have $[G,C_k] = G^{(\mathbb I_{x,n})^k}$.
...
\begin{remark}
  Recall that for any simplicial complex $K$, we 
  can define the free group on the $q+1$-tuples of vertices of $K$, denoted 
  $C_q(K_0)$. 
  We have maps $\partial: C_q(K_0) \to C_{q-1}(K_0)$ on generators by 
  $\partial_q(k_0, \cdots k_q) = 
  \sum_{i = 0}^q (-1)^{i} (k_0 , \cdots , \widehat{k_i} , \cdots k_q$.


  Following theorem 6.10 of \cite{EilenbergSteenrod}, for the simplicial complex given by $V = \mathbb I_n^{\sim k}$, we can show that \rednote{TODO}
\end{remark}
