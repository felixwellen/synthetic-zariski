...
In this section we work in plain HoTT and study non-abelian torsors over an arbitrary type $X$. Our goal is to describe non-abelian torsors over $X$ trivialised by a given cover in terms of cocycles for this cover. 
...
Our goal is then to show $H^1(X,Y,G) = \check{H}^1(X,Y,G)$. First we define a map $\Phi$ from $H^1(X,Y,G)$ to $\check{H}^1(X,Y,G)$.
...
\item Given (ii) and (iii) and \Cref{familiarWithCohomology}, each $\Psi(\delta)$ is an element in $H^1(X,G)$. From (i) we see that we have $\Pi_{x:X}\psi(\delta_x)^{Y(x)}$, so by \Cref{CechConditionEquivalentStatement} we indeed get an element of $H^1(X,Y,G)$. 
\end{enumerate}
\end{proof}

Now we prove the two maps are inverse to each other.
\begin{lemma}\label{PhiCircPsi}
  For $\Phi,\Psi$ as above, we have that $\Phi\circ \Psi$ is the identity on $\check H^1(X,Y,G)$. 
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{PSiCircPhi}
  For $\Phi,\Psi$ as above, we have that $\Psi\circ \Phi$ is the identity on $H^1(X,Y,G)$. 
\end{lemma}
\begin{proof}
Let $\alpha:\Pi_{x:X}BG(x)$ with $\beta:\Pi_{x:X}\alpha(x)^{Y(x)}$. We must show that for all $x:X$ we have 
...
Next we check that $g(f(p)) = p$, for any $p:\alpha(x)$. Since we want to prove a proposition we can assume $y:Y(x)$, then we need to prove that $\beta_x(y)\cdot (\beta_x(y)^{-1}\cdot p) = p$ which is clear. Finally to check that for any $\gamma:\psi(\phi(\alpha,\beta)_x)$, we have $f(g(\gamma)) = \gamma$, using that $g(\gamma)$ can be computed using any $y:Y(x)$, it is sufficient to prove that $\beta_x(y)^{-1}\cdot \beta_x(y) \cdot \gamma(y) = \gamma(y)$, which is indeed the case. 
\end{proof}
