...
Assume given $S$ compact Hausdorff and for each $x:S$ a tower:
...
Since $\Sigma(x,y:T). p(x)=p(y)$ is compact Hausdorff we can apply \Cref{compact-hausforff-countable-cover} to get a $j:\N$ such that:
\[\forall(x,y:T). p(x)=p(y) \to g(x) =_{I_j(x)} g(y)\]
which gives a factorisation of $f$ through $I_j$.
\rednote{Note that $\Sigma(x,y:T). p(x)=p(y)$ is even Stone, so here we also don't really need the result for compact Hausdorff types.}
\end{proof}
