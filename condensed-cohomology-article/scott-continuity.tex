...
\item For all $k:\N$ there exists a section in $\Pi_{x:S}T_k(x)$.
\end{itemize}
\end{lemma}

\begin{proof}
  By assumption, the projection 
  $\pi: \Sigma_{x:S} T(x) \to S$ is surjective.
...
Here we give generalities on overtly discrete types. Their definition is in some sense dual to that of Stone spaces. Most results come from \cite{synthetic-stone-duality}.
...
%Then:
%\[\mathrm{colim}_k \mathrm{Fin}(l_k)  =  \left(\Sigma_{k:\N} \mathrm{Fin}(l_k)\right) / L\]
%where $L$ is the equivalence relation generated by $(k,x) \sim (k+1,f_k(x))$. But $\Sigma_{k:\N} \mathrm{Fin}(l_k)$ is countable and the equivalence relation generated by a decidable relation on such a type is open.
%\end{itemize}
\end{proof}
...
Next result will not be used, we include it anyway as it gives nice intuitions about overtly discrete types and our cohomology results.
...
We already have proven all the results needed for our cohomology computations. In this section we prove the additional results needed to get all of Barton and Commelin's condensed type theory axioms. These axioms postulate two classes of types that have nice closure properties, and which interact well in the sense that they satisfy a form of Scott continuity. Our goal is to define two classes of type which satisfy all this in synthetic Stone duality. One of these class will be overtly discrete types, and \Cref{overtly-discrete-closure} already give all the desired closure property. But there is no hope of having the same properties for Stone spaces, as they are not closed under quotients. To fix this we consider compact Hausdorff spaces, which are precisely the quotients of Stone spaces by closed equivalence relations.
...
We need an auxiliary lemma to prove that compact Hausdorff spaces closed under sequential limits.

\begin{lemma}\label{sequential-limit-Hausdorff}
Given a tower of compact Hausdorff spaces $(C_k)_{k:\N}$, then there exists a tower of Stone spaces $(S_k)_{k:\N}$ with a map of tower $(p_k:S_k\to C_k)_{k:\N}$ such that each $p_k$ is surjective, and the induced map $\mathrm{lim}_kp_k:\mathrm{lim}_kS_k\to \mathrm{lim}_kC_k$ is also surjective. 
...
\begin{proof}
By definition of a Compact Hausdorff space, we can merely find a Stone space $S_0$ with a surjection $p_0:S_0\to C_0$. Next assume a surjection $p_k:S_k\to C_k$, and let us show there merely exists a Stone space $S_{k+1}$ with a surjection $p_{k+1}:S_{k+1}\to C_{k+1}$ making the appropriate square commutes.
%Using dependent choice, it is enough to show that we can merely extend such a tower $(S_k)_{k\leq n}$ to $(S_k)_{k\leq n+1}$.

Indeed there exists a Stone space $T$ with a surjection $q:T \to C_{k+1}$. Then we define $S_{k+1}$ by the following pullback square:
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
We define $p_{k+1} = q\circ \pi_1$ which is indeed surjective as $\pi_1$ is the pullback of a surjection and therefore surjective. By dependent choice this gives us a tower of Stone spaces $(S_k)_{k:\N}$ and with a level-wise surjection to $(C_k)_{k:\N}$

To check the induced map $\mathrm{lim}_kp_k:\lim_kS_k \to \lim_k C_k$ is surjective, we notice the for any $k>1$ the induced map:
\[S_{k+1} \to C_{k+1}\times_{C_k} S_k\] 
is surjective as it is a pullback of $q$, which implies the desired result by dependent choice.
\end{proof}
...
\item Clear from the definition of compact Hausdorff space as quotients.

\item Assume given a tower of compact Hausdorff spaces $(C_k)_{k:\N}$. Closed proposition are stable under sequential limits, so identity types in $\mathrm{lim}_kC_k$ are closed. By \cref{sequential-limit-Hausdorff} we get a surjection from a Stone space to $\mathrm{lim}_kC_k$,
\item Because for $C$ compact Hausdorff there is a surjection $S\to C$ with $S$ Stone, and Stone spaces have local choice.
\end{enumerate}
\end{proof}
...
Assume given $I$ overtly discrete and $C(i)$ compact Hausdorff depending on $i:I$. Then $\Pi_{i:I}C(i)$ is compact Hausdorff.
\end{proposition}
...
Extending the dual of Tychonov from Stone spaces to compact Hausdorff spaces only require generic reasoning on quotients.

\begin{proposition}[Tychonov's dual]
Assume given $C$ compact Hausdorff and $I(x)$ overtly discrete depending on $x:C$. Then $\Pi_{x:C}I(x)$ is overtly discrete.
\end{proposition}
...
\begin{proof}
Assume given a tower in ${\mathcal C}_\Chaus$ denoted by $(C_i,I_i)_{i:\N}$. By \Cref{sequential-limit-Hausdorff} we can consider $(S_i)_{i:\N}$ a tower of Stone spaces with $p_k:S_k\to C_k$ giving a level-wise surjection to the tower $(C_i)_{i:\N}$. We write $S=\mathrm{lim}_iS_i$ and $C=\mathrm{lim}_iC_i$, for $x:C$ we write $I(x) = \mathrm{colim}_iI_i(x_i)$ and finally we write $p =\mathrm{lim}_ip_i$. Then the canonical map:
\[\mathrm{colim}_i (\Pi_{x:C_i}I_i(x) ) \to \Pi_{x:C}I(x)\]
is equal to the canonical map:

\[ \left\{ \iota_i(f_i) : \mathrm{colim}_i\, \Pi_{x:S_i}\, I_i(p_i(x))\ |\ \mathrm{colim}_{j>i}\ Q_j(f_i)\right\} \longrightarrow \left\{f:\Pi_{x:S}\, I(p(x))\ |\ Q(f)\right\} \]
where we defined:
\begin{eqnarray}
 Q_j(f_i) &=& \Pi_{(x,y:S_j,p_j(x)=p_j(y))}\, \iota_j(f_i([x]))=\iota_j(f_i([y]))\nonumber\\
